...
//       | 4xx/5xx
//
// **이 파일은 프로파일 하나를 골라 엔진에 넘기는 일만 한다**(2026-09-06). 실행의 순서와
// 보안 규약은 전부 `_shared/aiFill/run.ts`가 갖고, 이 대상의 규격은 `profile.ts`가 갖는다.
// 둘을 가른 기준은 하나 — *무엇을 뽑는가*는 프로파일, *어떻게 뽑는가*는 엔진이다.
//
...
import { isCardKey, startupProfile } from './profile.ts'

Deno.serve(withCors((req: Request) => runAiFill(req, startupProfile, isCardKey)))
