...
//
// 인라인 전송은 원본·base64·JSON 사본이 함께 메모리에 서므로 우리 런타임에서는 14MB를
// 보수적 전환점으로 쓴다. 사업계획서·IR 자료·재무제표를 함께 읽히면 금세 걸린다. Files API는
// 자료를 먼저 올려 두고 `fileData`로 가리키므로 큰 문자열 사본이 사라진다.
//
