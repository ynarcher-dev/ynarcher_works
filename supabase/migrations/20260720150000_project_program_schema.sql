-- =====================================================================
-- [Phase 11] PROJECT 수행 사업 원장 — AC Program First 구조 이식
-- 배경: PROJECT 워크스페이스를 AC와 동일한 구조(대시보드/내 사업/전체 사업 + 리스트뷰 + 사업 상세 + 모듈 보드)로
--   재편한다. 원장은 AC와 물리적으로 분리하되 스키마 형태와 운영 규칙은 동일하게 유지하여
--   프론트 공용 모듈(features/program)이 테이블명 주입만으로 재사용되도록 한다.
--
-- 범위:
--   - 신규 8테이블: project_programs / project_program_modules / project_program_module_assignees /
--     project_program_managers / project_program_departments / project_program_participants /
...
--   - 소유 워크스페이스: project
--   - 데이터 등급: Internal (수행 사업 정보. 게스트 전면 차단)
--   - 접근 주체: 내부 사용자만. 외부 스타트업/전문가/게스트 정책 미부여 → default deny
--   - Scope 기준: global 또는 program 단건(app.can_access_ws_program('project', ...))
...
create unique index if not exists idx_project_programs_code on public.project_programs (code);

-- (2) 모듈 인스턴스 ---------------------------------------------------------
create table if not exists public.project_program_modules (
  id                 uuid primary key default gen_random_uuid(),
...
create index if not exists idx_project_program_modules_program on public.project_program_modules (program_id);
-- 모듈명은 사업 내에서 유일(공백·대소문자 정규화 비교).
create unique index if not exists uq_project_program_modules_title
  on public.project_program_modules (program_id, lower(btrim(title)))
  where title is not null;
-- 성과/KPI 모듈은 사업당 1개(현재 PROJECT는 CUSTOM_ACTIVITY만 노출하나 규칙은 AC와 동일하게 유지).
create unique index if not exists uq_project_program_modules_outcomes_singleton
  on public.project_program_modules (program_id)
  where module_type = 'OUTCOMES';

-- (3) 모듈 담당자 -----------------------------------------------------------
create table if not exists public.project_program_module_assignees (
  id                uuid primary key default gen_random_uuid(),
...
create index if not exists idx_project_module_assignees_module
  on public.project_program_module_assignees (program_module_id);
create index if not exists idx_project_module_assignees_user
  on public.project_program_module_assignees (user_id);

-- (4) 부서 구성 / 담당자 배치 ------------------------------------------------
create table if not exists public.project_program_departments (
  id                  uuid primary key default gen_random_uuid(),
  program_id          uuid not null references public.project_programs(id) on delete cascade,
  ...
  created_at          timestamptz not null default now(),
  constraint project_program_departments_prog_ver_dept_key
    unique (program_id, org_version_id, department_id)
);

create index if not exists idx_project_program_departments_program
  on public.project_program_departments (program_id);
create index if not exists idx_project_program_departments_prog_ver
  on public.project_program_departments (program_id, org_version_id);

-- 동일 (program_id, user_id) 복수 행 허용 — 기간 세그먼트 모델(AC와 동일).
create table if not exists public.project_program_managers (
  id              uuid primary key default gen_random_uuid(),
  program_id      uuid not null references public.project_programs(id) on delete cascade,
  user_id         uuid not null references public.users(id),
  ...
  assigned_by     uuid references public.users(id),
  assigned_at     timestamptz not null default now(),
  constraint project_program_managers_alloc_chk  check (allocation_rate between 1 and 100),
  constraint project_program_managers_period_chk check (end_date >= start_date)
);

create index if not exists idx_project_program_managers_program on public.project_program_managers (program_id);
create index if not exists idx_project_program_managers_user    on public.project_program_managers (user_id);

-- (5) 참가자 / 타임라인 / 커스텀 활동 ----------------------------------------
create table if not exists public.project_program_participants (
  id         uuid primary key default gen_random_uuid(),
  program_id uuid not null references public.project_programs(id) on delete cascade,
...
create index if not exists idx_project_program_participants_program
  on public.project_program_participants (program_id);

create table if not exists public.project_program_timeline_items (
  id                uuid primary key default gen_random_uuid(),
  program_id        uuid not null references public.project_programs(id) on delete cascade,
  program_module_id uuid references public.project_program_modules(id),
  source_table      text,
  source_id         uuid,
  item_type         text,
  title             text not null,
...
create index if not exists idx_project_timeline_items_program
  on public.project_program_timeline_items (program_id);

create table if not exists public.project_custom_activities (
  id                uuid primary key default gen_random_uuid(),
  program_id        uuid not null references public.project_programs(id) on delete cascade,
  program_module_id uuid references public.project_program_modules(id),
...
create index if not exists idx_project_custom_activities_program
  on public.project_custom_activities (program_id);
create index if not exists idx_project_custom_activities_module
  on public.project_custom_activities (program_module_id);

-- (6) updated_at 트리거 -----------------------------------------------------
drop trigger if exists trg_project_programs_updated_at on public.project_programs;
create trigger trg_project_programs_updated_at
  before update on public.project_programs
  for each row execute function app.set_updated_at();

drop trigger if exists trg_project_program_modules_updated_at on public.project_program_modules;
create trigger trg_project_program_modules_updated_at
  before update on public.project_program_modules
  for each row execute function app.set_updated_at();

drop trigger if exists trg_project_program_participants_updated_at on public.project_program_participants;
create trigger trg_project_program_participants_updated_at
  before update on public.project_program_participants
  for each row execute function app.set_updated_at();

drop trigger if exists trg_project_custom_activities_updated_at on public.project_custom_activities;
create trigger trg_project_custom_activities_updated_at
  before update on public.project_custom_activities
  for each row execute function app.set_updated_at();

-- (7) 사업코드 자동 부여 — 6자리 영숫자 난수(public.gen_program_code() 재사용) ---
create or replace function public.project_programs_assign_code()
returns trigger
language plpgsql
...
begin
  ...
  loop
    ...
    exit when not exists (select 1 from public.project_programs where code = candidate);
  end loop;
  new.code := candidate;
  return new;
end;
...
create trigger trg_project_programs_assign_code
  ...
  for each row execute function public.project_programs_assign_code();

-- (8) 모듈 담당자 풀 소속 강제 트리거 ----------------------------------------
create or replace function public.enforce_project_module_assignee_in_pool()
returns trigger
language plpgsql
security definer
...
begin
  ...
  from public.project_program_modules where id = new.program_module_id;
  ...
  if not exists (
    select 1 from public.project_program_managers pm
...
drop trigger if exists trg_project_module_assignee_pool on public.project_program_module_assignees;
create trigger trg_project_module_assignee_pool
  before insert or update on public.project_program_module_assignees
  for each row execute function public.enforce_project_module_assignee_in_pool();
...
alter table public.project_program_modules           enable row level security;
alter table public.project_program_module_assignees  enable row level security;
alter table public.project_program_departments       enable row level security;
alter table public.project_program_managers          enable row level security;
alter table public.project_program_participants      enable row level security;
alter table public.project_program_timeline_items    enable row level security;
alter table public.project_custom_activities         enable row level security;

-- 원장(스코프 컬럼 = id)
drop policy if exists project_programs_select on public.project_programs;
create policy project_programs_select on public.project_programs for select
  using (app.can_read_workspace('project') and app.can_access_ws_program('project', id));

drop policy if exists project_programs_insert on public.project_programs;
create policy project_programs_insert on public.project_programs for insert
  with check (app.can_write_workspace('project') and app.can_access_ws_program('project', id));

drop policy if exists project_programs_update on public.project_programs;
create policy project_programs_update on public.project_programs for update
  using (app.can_write_workspace('project') and app.can_access_ws_program('project', id))
  with check (app.can_write_workspace('project') and app.can_access_ws_program('project', id));
...
begin
  foreach t in array array[
    'project_program_modules',
    'project_program_participants',
    'project_program_timeline_items',
    'project_custom_activities'
  ] loop
    execute format('drop policy if exists %I on public.%I', t || '_select', t);
    execute format(
      'create policy %I on public.%I for select using (app.can_read_workspace(''project'') and app.can_access_ws_program(''project'', program_id))',
      t || '_select', t);

    execute format('drop policy if exists %I on public.%I', t || '_insert', t);
    execute format(
      'create policy %I on public.%I for insert with check (app.can_write_workspace(''project'') and app.can_access_ws_program(''project'', program_id))',
      t || '_insert', t);

    execute format('drop policy if exists %I on public.%I', t || '_update', t);
    execute format(
      'create policy %I on public.%I for update using (app.can_write_workspace(''project'') and app.can_access_ws_program(''project'', program_id)) with check (app.can_write_workspace(''project'') and app.can_access_ws_program(''project'', program_id))',
      t || '_update', t);
  end loop;
end;
$$;
...
drop policy if exists project_program_departments_select on public.project_program_departments;
create policy project_program_departments_select on public.project_program_departments for select
  using (app.can_read_workspace('project') and app.can_access_ws_program('project', program_id));

drop policy if exists project_program_managers_select on public.project_program_managers;
create policy project_program_managers_select on public.project_program_managers for select
  using (app.can_read_workspace('project') and app.can_access_ws_program('project', program_id));

-- 모듈 담당자는 상위 모듈 경유 판정.
drop policy if exists project_program_module_assignees_select on public.project_program_module_assignees;
create policy project_program_module_assignees_select on public.project_program_module_assignees for select
  using (app.can_read_workspace('project') and exists (
    select 1 from public.project_program_modules pm
    where pm.id = program_module_id and app.can_access_ws_program('project', pm.program_id)));

drop policy if exists project_program_module_assignees_insert on public.project_program_module_assignees;
create policy project_program_module_assignees_insert on public.project_program_module_assignees for insert
  with check (app.can_write_workspace('project') and exists (
    select 1 from public.project_program_modules pm
    where pm.id = program_module_id and app.can_access_ws_program('project', pm.program_id)));

drop policy if exists project_program_module_assignees_update on public.project_program_module_assignees;
create policy project_program_module_assignees_update on public.project_program_module_assignees for update
  using (app.can_write_workspace('project') and exists (
    select 1 from public.project_program_modules pm
    where pm.id = program_module_id and app.can_access_ws_program('project', pm.program_id)))
  with check (app.can_write_workspace('project') and exists (
    select 1 from public.project_program_modules pm
...
create or replace function public.set_project_program_staffing(
  p_program_id  uuid,
  p_departments jsonb,
  p_managers    jsonb
)
...
begin
  ...
  if not (
    app.is_admin()
    or (app.can_write_workspace('project') and app.can_access_ws_program('project', p_program_id))
  ) then
  ...
  if v_dep_count = 0 and v_mgr_count = 0 then
    delete from public.project_program_managers    where program_id = p_program_id;
    delete from public.project_program_departments where program_id = p_program_id;
    return;
  end if;
  ...
  select start_date, end_date into v_prog_start, v_prog_end from public.project_programs where id = p_program_id;
  ...
  delete from public.project_program_managers    where program_id = p_program_id;
  delete from public.project_program_departments where program_id = p_program_id;
  insert into public.project_program_departments (program_id, org_version_id, department_id, kind, collaboration_ratio)
  select p_program_id, d.org_version_id, d.department_id, d.kind::public.program_department_kind, d.collaboration_ratio
  from jsonb_to_recordset(p_departments) as d(org_version_id uuid, department_id uuid, kind text, collaboration_ratio int);
  insert into public.project_program_managers
...
grant execute on function public.set_project_program_staffing(uuid, jsonb, jsonb) to authenticated;

comment on function public.set_project_program_staffing(uuid, jsonb, jsonb) is
  'PROJECT 사업 부서+담당자 배치 org 버전(단계)별 독립 검증·원자 교체. set_program_staffing(AC)의 project 원장 대응.';

-- (11) 모듈 인스턴스 생성/수정 RPC -------------------------------------------
create or replace function public.set_project_program_module(
  p_program_id         uuid,
  p_module_id          uuid,
  p_module_type        text,
  p_title              text,
...
begin
  ...
  if v_id is not null and not exists (
    select 1 from public.project_program_modules where id = v_id and program_id = p_program_id
  ) then
  ...
  if p_module_type = 'OUTCOMES' and v_id is null and exists (
    select 1 from public.project_program_modules where program_id = p_program_id and module_type = 'OUTCOMES'
  ) then
  ...
  if v_ps is not null and v_pe is not null then
    ...
    from public.project_programs where id = p_program_id;
  ...
  if v_id is null then
    insert into public.project_program_modules
    ...
    update public.project_program_modules set
      title              = v_title,
  ...
  delete from public.project_program_module_assignees where program_module_id = v_id;
  if p_assignee_user_ids is not null then
    insert into public.project_program_module_assignees (program_module_id, user_id, assigned_by)
...
grant execute on function public.set_project_program_module(uuid, uuid, text, text, text, text, text, jsonb, uuid[]) to authenticated;

comment on function public.set_project_program_module(uuid, uuid, text, text, text, text, text, jsonb, uuid[]) is
  'PROJECT 운영 모듈 인스턴스 생성/수정 + 담당자 전량 교체(원자). set_program_module(AC)의 project 원장 대응.';
