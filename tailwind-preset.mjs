  ...
  theme: {
    extend: {
      ...
      fontSize: {
        ...
        // 13px 단(2026-09-06 폐지). 종전에는 컨트롤 라벨(`body-sm`)과 카드 안 표(`table-card`)를
        // 본문(14px)보다 한 단계 눌러, 조작 요소·부속 표가 읽을거리보다 앞으로 튀어나오지 않게
        // 했다. 그 1px은 눌러 준 만큼의 값을 하지 못했다 — 14와 13은 나란히 놓여야 겨우 갈리는
        // 차이라 위계로는 읽히지 않고, 대신 같은 화면에서 자리마다 본문 크기가 미세하게
        // 어긋나는 것으로만 보였다. 조작 요소를 뒤로 물리는 일은 이미 색과 굵기가 하고 있다.
        //
        // **토큰은 합치지 않고 값만 올린다.** 이름이 남아 있어야 "여기는 컨트롤 라벨", "여기는
        // 카드 안 표"라는 자리 표시가 call site에 남고, 나중에 이 단을 되살리거나 다르게
        // 조정할 때 고칠 곳이 이 두 줄뿐이다. 이름을 지워 `body`로 흡수하면 그 자리들이 전부
        // 뭉뚱그려져 되돌릴 근거가 사라진다. letterSpacing이 서로 다른 것도 그대로 둔다 —
        // 표는 자간을 좁히지 않아야 숫자 열이 흔들리지 않는다.
        'body-sm': ['0.875rem', { lineHeight: '1.4', letterSpacing: '-0.01em' }],
        'table-card': ['0.875rem', { lineHeight: '1.4', letterSpacing: '0em' }],
      ...
      spacing: {
        ...
        // 상세 우측 패널 안에 한 줄로 서는 목록 행(자료·회의록·전자결재 …)의 최소 높이.
        // 값의 근거는 첨부 행이다 — 아이콘 버튼(icon-card 28) 위아래로 8px씩 + 테두리 2px.
        // 액션이 없는 행(회의록·전자결재 링크)은 글자만 있어 39px로 주저앉는데, 같은 열에
        // 세로로 쌓이는 상자의 높이가 내용물에 따라 갈리면 목록이 아니라 조각으로 보인다.
        'panel-row': '2.875rem', // 46px
      },
      /**
      ...
      maxWidth: {
        ...
        // 1792px — 폭을 우리가 정하지 못하는 대화. 2xl과 가르는 것은 중요도가 아니라 **폭을
        // 정하는 주체**다. 2xl까지는 우리가 정한 열 구성이 폭을 정하지만, 격자는 열 수가
        // 데이터(자료 건수·카드 수)라, 좌우로 옮기는 창(TransferPanes)은 한 줄에 서는 값의
        // 개수가 원장이 정한 것이라 우리가 못 정한다 — 좁히면 줄어드는 것이 여백이 아니라
        // 한 번에 보이는 칸 수이고 잘리는 것은 확인하러 연 값이다.
        //
        // 1400px에서 올렸다(2026-09-08 사용자 지적) — 참조 자료에 위치 태그가 붙으면서 파일명
        // 칸이 태그와 폭을 나눠 갖게 되어, 같은 비율에서 이름이 더 잘렸다. 여기서 넓히는 것이
        // 비율을 다시 나누는 것보다 먼저인 이유는 잘리는 쪽과 좁아지는 쪽이 **둘 다 데이터**라,
        // 안에서 나누면 한쪽을 살릴 때 다른 쪽이 죽기 때문이다.
        //
        // 상한을 아주 없애지는 않는다. 격자 폭이 곧 창 폭이 되면 초광폭 화면에서 카드 열
        // 사이가 손가락 두 개만큼 벌어져, 어느 칸이 어느 카드인지 눈이 가로로 훑어 찾아야 한다.
        'modal-3xl': '112rem',
      },
      zIndex: {
        base: '0',
      ...
      boxShadow: {
        ...
        // 'pinned'(고정 열 우측 seam)는 폐지했다(2026-09-02) — 가로로 번지는 그림자가 표를
        // 세로로 가르는 띠로 읽혀, 스크롤 위치와 무관하게 없는 경계를 그렸다. 고정 열은
        // 불투명 배경만으로 스크롤되는 셀을 덮는다.
      },
    },
  },
}
