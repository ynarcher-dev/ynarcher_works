  ...
  theme: {
    extend: {
      ...
      maxWidth: {
        ...
        // 1792px — 격자(행×열 모두 데이터인 표)를 품는 대화. 2xl과 가르는 것은 중요도가
        // 아니라 **폭을 정하는 주체**다. 2xl까지는 우리가 정한 열 구성이 폭을 정하지만,
        // 격자는 열 수가 데이터(자료 건수·카드 수)라 우리가 못 정한다 — 좁히면 줄어드는
        // 것이 여백이 아니라 한 번에 보이는 칸 수다.
        //
