  ...
  theme: {
    extend: {
      colors: {
        ...
        // 입력 검증 실패 보더가 더 이상 brand 램프를 빌려 쓸 수 없기 때문이다.
        //
        // 700~900은 구 brand 램프에서 승계한 벽돌빛(#B31A0F 계열)이었다. 이 값들은 채도가 낮고
        // 노란기가 돌아 흰 배경에서 붉은 신호가 아니라 갈색·버건디처럼 읽혔다 — NEW 배지·[공지]
        // 말머리·건수처럼 "여기를 보라"는 자리들이 정작 눈에 띄지 않았다. 같은 자리를 지키되
        // 색상각을 순적색(0°)으로 세우고 채도를 올려 쨍하게 바꾼다(2026-08-03).
        // 대비는 그대로 유지된다 — 700은 흰 배경 위 글자·흰 글씨 배경 양쪽 모두 4.7:1(AA).
        danger: {
          DEFAULT: '#EF4444',
          subtle: '#FFF5F5',
          border: '#FFE2E2',
