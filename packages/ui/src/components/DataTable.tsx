              ...
              return (
                <th
                  ...
                  className={cn(
                    ...
                    //
                    // `layout="fixed"`에서는 **종류가 없는 열의 머리글도** 함께 접는다(2026-09-08).
                    // 위 근거("머리글이 열 폭을 이긴다")가 성립하는 것은 자동 레이아웃뿐이다 —
                    // `table-fixed`에서 열 폭은 선언값이 정하고 내용은 그 폭을 밀지 못하므로, 접지
                    // 않으면 머리글이 열을 넓히는 대신 그냥 말줄임된다. 실제로 M&A 퀵 리뷰의 재무
                    // 표(종류를 적지 않은 값 열이 여럿인 표)에서 `현금성자산`이 `현금성…`으로 잘렸다.
                    // 열이 늘어날 수 없는 자리에서 nowrap은 폭을 지키는 장치가 아니라 글자를 지우는
                    // 장치다.
                    (spec?.width || fixed) && 'whitespace-normal break-keep',
