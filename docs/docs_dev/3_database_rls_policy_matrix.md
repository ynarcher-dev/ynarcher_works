...
5. **물리 삭제 금지**: 비즈니스 데이터의 임의 삭제를 방지하기 위해 `DELETE` 정책은 원천 차단하거나 소프트 딜리트(`is_deleted = true` 또는 `deleted_at IS NOT NULL`) 필드 수정을 유도하는 `UPDATE` 권한으로 대체한다.
   * **예외를 여는 방식**: 그릇에 해당하는 레코드(운영 모듈 인스턴스 등)를 물리 삭제해야 할 때도 `DELETE` 정책은 만들지 않는다. 정책을 열면 정당 경로 밖에서도 지울 수 있어 조건 강제가 무의미해지므로, 자체 인가하는 RPC 하나만 두고 그 안에서 권한·확인 문구·잔존 데이터를 검사한다(`delete_program_module`, 20260902180000). 딸린 업무 기록이 한 건이라도 살아 있으면 거부하는 것이 이 예외의 경계다.
