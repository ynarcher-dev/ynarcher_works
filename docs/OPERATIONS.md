...
아래는 사용 가능한 명령 목록이며 매 작업마다 전부 실행하는 체크리스트가 아닙니다. 실행 범위와 재검사 기준은 [작업 규칙 §4](./docs_master/CLAUDE.md)의 변경 범위별 검증·검사 결과 재사용 규칙을 따릅니다. 문서만 수정하면 diff와 필요한 링크 확인으로 마칩니다. 전체 게이트는 전역 영향·불명확한 실패·사용자 요청 또는 CI에서 수행합니다.
...
| DB·RLS | 마이그레이션 보안 게이트, 최종 변경 묶음에 `pnpm test:db <관련 SQL 파일>`(§4.3), 운영 반영 전 diff. 공통 권한 헬퍼·전역 스키마 영향이면 전체 SQL 검사 |
...
표의 사례는 변경 영향에 맞춰 선택하며 매번 모두 실행하지 않습니다. 테스트 통과와 운영 가능은 같은 뜻이 아닙니다. 외부 공급자, 운영 DB, IAM, DNS, 백업 복구 확인은 해당 운영·배포 작업 범위일 때 수행합니다.
...
* **실행 횟수**: DB 변경이 있을 때 최종 변경 묶음에서 한 번 실행하고, 성공 후에는 DB·관련 테스트·설정 변경이나 새로운 결함 근거가 있을 때만 재실행합니다. 같은 결과를 리뷰에서도 재사용합니다. 이 명령 자체가 전체 재생을 하므로 검증 목적으로 `db:reset`을 앞서 반복하지 않습니다. 일부 SQL 선택은 테스트 범위만 줄이며 전체 재생 비용은 남습니다.
...
4. 최종 변경 묶음에 `pnpm test:db <관련 SQL 파일>`로 격리 재생과 관련 회귀를 한 번 확인합니다(§4.3). 공통 권한 헬퍼·전역 스키마 변경이면 전체 SQL 검사를 실행합니다. `db:reset`은 개발 DB 상태를 실제로 초기화해야 할 때만 별도로 사용합니다.
5. 배포·운영 반영 범위라면 스테이징에서 영향받은 역할별 접근을 확인합니다.
6. 운영 project ref와 백업 시점을 확인하고 **사용자 승인 후** 반영합니다.
7. 적용 결과와 롤백·보정 방법을 기록합니다.

```bash
pnpm db:migration <name>
pnpm test:db <관련 SQL 파일> # 격리 스택 재생 + 관련 pgTAP (§4.3)
pnpm db:baseline:check    # cutoff 이하 이력 불변성
pnpm db:baseline:verify   # 베이스라인 산출물·생성 로직 변경 또는 명시적 검증 요청 시만
```
