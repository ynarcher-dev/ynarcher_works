...
* **[3_7_4_management_partners.md](../docs_planning/3_7_4_management_partners.md) (MANAGEMENT 거래처 정보 기획서)**
  * **의미**: 회사가 대금을 주고받는 상대(거래처)를 코드·상호·구분(개인/법인)·등록번호·계좌(은행·계좌번호·예금주)·증빙 서류 2종으로 담는 원장 기획서입니다. 거래처 코드는 영문 2글자 접두어만 담당자가 정하고 일련번호 5자리는 원장 트리거가 접두어 단위 잠금 아래에서 매기며 발급 후 불변입니다. 구분 하나가 등록번호의 뜻(사업자등록번호 10자리 ↔ 생년월일 8자리)과 서류 이름(사업자등록증 ↔ 신분증)을 함께 가르고, 계좌 세 값은 함께 서거나 함께 비며, 증빙은 비공개 버킷에 두고 열람 시 `access_logs` 적재를 선행합니다. NETWORKS 외주/거래 마스터(`vendors`)와 합치지 않는 이유(노출 범위)와 OFFICE 전사 조회 목록의 선행 조건(마스킹 정책·서버 강제 다운로드)을 함께 정의합니다.
