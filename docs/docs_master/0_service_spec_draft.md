...
* **[3_1_4_approval_budget_expense.md](../docs_planning/3_1_4_approval_budget_expense.md) (전자결재 예산·지출 연동 기획서)**
  * **의미**: 2026-09-11 확정. 품의서에 층(트리) 있는 예산표를 두고(층 이름·층 수는 문서마다 자유, 숫자 열은 양식이 고정, 금액은 맨 아래 줄에만), 사업 지출결의서는 승인된 품의의 예산 줄을 골라 승인 시 차감하며, 이익·이익률은 품의 금액과 승인 지출로 계산합니다. 예산 변경은 별도 양식으로 신청해 승인 시 원 품의에 반영하고 이력을 남깁니다. 초과·마이너스는 막지 않고 표시만 하며 결재자의 반려가 거릅니다. 송금 요청의 거래처는 원장에서만 고르고(은행·계좌·예금주 잠김, 거래처 하나 = 계좌 하나), 즉석 등록은 원장에 먼저 들어가되 "확인 전" 딱지가 붙습니다. 양식 틀은 그대로 두고 칸 종류 셋과 장부 하나를 더합니다.
...
* **[3_3_workspace_networks.md](../docs_planning/3_3_workspace_networks.md) (NETWORKS 워크스페이스 상세 기획서)**
  ...
  * **[3_3_2_networks_dashboard.md](../docs_planning/3_3_2_networks_dashboard.md) (NETWORKS 대시보드 및 최근 업로드 현황)**: 네트워크 자산의 규모·구성·성장 추이·데이터 건강도 인포그래픽과 9종 구분 통합 최근 업로드 리스트 화면을 명세합니다.
  ...
  * **[3_3_8_ledger_identity_dedup.md](../docs_planning/3_3_8_ledger_identity_dedup.md) (원장 식별 규칙 — 확실한 키는 DB가 막고, 의심 조합은 화면이 한 번 멈춘다)**: 2026-09-11 확정. 사람·회사 원장 넷(`startups`·`networks`·`ma_sellers`·`ma_buyers`)과 내부 계정의 중복 기준을 두 층으로 가릅니다 — 확실한 키(스타트업·M&A 사업자등록번호, 사람의 이름 + 연락처, 내부 이메일)는 DB 유일 인덱스·트리거가 거절하고, 이름·이메일·전화 두 칸 일치는 화면이 한 번 멈춥니다. 사업자등록번호의 저장 모양·체크섬·보육/투자 전환 필수, M&A 미연결 행의 스타트업 연결 요구(반대 방향은 기밀 때문에 묻지 않음), 게스트 발급의 이름 대조, 규칙 이전 중복 7행의 하드 딜리트 근거를 담습니다.
