...
MANAGEMENT 권한이 없는 임직원에게 물품과 사진만 노출하는 `portable_assets` 뷰, 지사 탭·검색·표·상세 모달 사양을 담습니다. 2026-09-06에 반출 예약 기능을 도입하지 않기로 확정하여 `assets.requires_approval`과 흐름 트리거·함수를 걷고 `asset_checkouts`는 기록 보존을 위해 `_retired_asset_checkouts`로 개명했습니다.
* **[3_1_3_approval_return_flow.md](../docs_planning/3_1_3_approval_return_flow.md) (전자결재 반려·보완·재상신 기획서)**
  * **의미**: 2026-09-10에 반려와 보완을 분리했습니다. 반려는 결재를 종결하고, 보완은 현재 결재 자리에서 노란색으로 멈춰 기안자에게 수정을 연 뒤 재상신 시 그 자리부터 이어갑니다. 이미 승인된 도장은 회차 이력으로 유지합니다. 돌아갈 순번·기안자 경유·합의 초기화를 고르는 종전 되돌림은 폐기했으며, 같은 사람은 독립된 결재 자리로 N회 배치할 수 있습니다. 참조자는 결재선과 분리됩니다.
...
* **[3_3_workspace_networks.md](../docs_planning/3_3_workspace_networks.md) (NETWORKS 워크스페이스 상세 기획서)**
  ...
  * **[3_3_7_ai_fill_visual_read.md](../docs_planning/3_3_7_ai_fill_visual_read.md) ('AI 작성하기' 재설계 — 눈으로 읽고 자료는 상·하 두 칸)**: 2026-09-09 재설계의 정본입니다. PDF·이미지를 글자로 뽑지 않고 원본 그대로 모델이 읽는 결정(눈으로 보기 대 VLM 마크다운 비교), 카드×자료 격자를 걷고 읽을 자료(상)·읽지 않을 자료(하) 두 칸으로 가른 화면, 파일명으로 판정한 자료 종류를 프롬프트 꼬리표로 붙이는 규칙, 자료 한 벌 위의 탐색 축 묶음과 컨텍스트 캐시, 가져간 것과 버린 것의 목록을 규정합니다. [3_3_5](../docs_planning/3_3_5_startup_ai_fill.md)의 §4.2·§8.3·§16.3을 대체하며 STARTUP과 M&A 셀러 퀵 리뷰에 함께 적용됩니다.
...
* **[3_7_1_management_performance_kpi.md](../docs_planning/3_7_1_management_performance_kpi.md) (MANAGEMENT 조직 성과·KPI 관리 기획서)**
  * **의미**: 조직 원장과 KPI를 동일 적용기간의 1:1 스냅샷으로 묶고 조직개편 때 함께 발행하는 성과관리 정본입니다. 발행된 스냅샷은 이전 버전과 계산상 단절되며, 재사용 구성 원형에 버전별 지표·목표·점수 규칙을 채웁니다. 개인 KPI는 임직원에게 직접 귀속해 부서이동과 무관하게 유지하고, 부서 KPI는 기간형 소속을 통해 이동일부터 새 부서 것을 상속합니다. MANAGEMENT의 구성·부서 할당·개인 할당·실적 평가와 OFFICE의 `나의 KPI`·`소속 부서 KPI` 대시보드까지 정의합니다.
* **[3_7_2_management_assets.md](../docs_planning/3_7_2_management_assets.md) (MANAGEMENT 자산 관리 기획서)**
  * **의미**: 자산 조회 표를 취득부터 폐기까지 추적하는 자산 원장으로 확장하는 기획서입니다. 지사 원장(branches)에 필수 귀속시켜 지사 탭이 곧 화면 구조가 되게 하고, 
분류(구매·리스·렌탈·기타)·상태 4종·할당 대상(임직원)·취득/폐기일자·금액·OFFICE 공개 여부(토글)를 원장 필드로 정의합니다. 
등록자 열 없이 수정일만 남기는 표 규격, OFFICE 자산 현황의 노출을 정하는 `is_portable` 기준값(2026-09-06에 화면 이름을 `OFFICE 자산 현황에 공개`로 바로잡음), 새 테이블 없이 `public.assets`를 확장하는 마이그레이션 설계를 포함합니다.
...
* **[3_9_1_guest_unified_account.md](../docs_planning/3_9_1_guest_unified_account.md) (게스트 통합 계정 기획서)**
  * **의미**: 게스트 계정의 단일 원천입니다. 계정을 **사업 단위가 아니라 대상 단위**로 발급하고(키는 이메일이 아니라 원장 행), 자격증명을 초대 행에서 신설 원장 `guest_credentials`로 옮기며, 로그인을 이메일·비밀번호 2요소로 좁힙니다. 사업코드는 안내·식별용으로만 남고, 세션 맥락 클레임을 `context_type`+`context_id`로 일반화해 FUND 연동에 대비합니다. 접근 기간은 계정이 아니라 참여 줄이 갖고, 권한은 **발급=내부 전원 / 매핑·기간·차단=사업 담당자 / 정지·해제=ADMIN / 비밀번호=본인만** 셋으로 갈립니다. WORKS(OFFICE·ADMIN·사업 3종) + GUEST 앱 공통 적용. *계정 키를 원장 행에서 사람으로 옮기는 개정은 3_9_2가 승계합니다.*
* **[3_9_2_external_portal_expansion.md](../docs_planning/3_9_2_external_portal_expansion.md) (외부 포털 확장 기획서)**
  * **의미**: 외부인이 들어오는 자리 전체의 단일 원천입니다. 받는 곳을 AC 하나에서 **AC·FUND·M&A 셋**으로 넓히되 앱은 하나로 두고(신원·셸은 같고 화면 종류만 갈립니다), 계정 단위를 **원장 행에서 사람으로** 옮겨 같은 회사에 담당자 여럿을 발급할 수 있게 합니다. 발급 창구가 보는 범위는 사업이 아니라 **인격의 출처 원장**으로 좁히고(`ma_sellers` 인격이 AC 창구에 서지 않습니다), `guest_identities`의 SELECT 정책도 원장별로 좁혀 *누가 매각 검토 중인가*가 전 직원에게 새는 길을 막습니다. M&A의 자료 선별은 파일을 옮기거나 복제하지 않고 **여는 목록**(연결 표)으로 표현하며 — *전량이면 함수, 고르면 표* — 단계 축(`stage`/`min_stage`) 하나로 NDA 전후를 가릅니다. FUND의 외부 대상은 포트폴리오사 하나이고 온기보고 항목은 ADMIN 템플릿이 소유합니다. 밖에서 부르는 이름은 **와이앤아처 GUEST**이며(2026-09-08에 `와이앤아처 포털`로 바꿨다가 2026-09-09에 되돌렸습니다) `guest` 권한 키·`external_*` enum은 그대로입니다.
