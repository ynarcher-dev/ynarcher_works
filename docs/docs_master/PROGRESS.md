...
- [ ] NETWORKS 대시보드 탭 구현 (규모·구성·추이·데이터 건강도 인포그래픽 + 8종 통합 최근 업로드 리스트) — [3_3_2](../docs_planning/3_3_2_networks_dashboard.md) <!-- 기획 완료. fn_get_networks_dashboard_summary/fn_get_networks_recent_uploads RPC(SECURITY DEFINER, 보안 게이트) + 대시보드 위젯/차트(chart.1~8 규칙)/최근 업로드 리스트(마스킹·배치별 보기) 구현은 후속. 활동·만족도(_activity/_satisfaction)는 임시값이라 제외, 매칭 가능여부(match_available)만 편입 -->
