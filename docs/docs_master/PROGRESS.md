...
- [ ] **[선행]** entity_contributions RLS를 워크스페이스 인지형으로 정합화 <!-- 현재 SELECT/INSERT가 `entity_table='program'이면 ac, 그 외는 networks`뿐이다. 두 가지 문제: (1) MANAGEMENT(임직원)·FUND·OFFICE(게시판) 로그를 트리거로 남기기 시작하면 networks 열람자에게 전부 보인다 — 워크스페이스별 판정 필요. (2) M&A·PROJECT 프로그램이 AC와 같은 entity_table='program' 값을 공유해(programContributions.ts:9) 정책의 ac 분기에 걸리므로, M&A/PROJECT 사용자는 자기 사업의 변동 이력을 못 본다. entity_table 값 분리('ma_program'/'project_program')는 기존 행 백필이 필요하고, app.can_access_ws_program(ws_key,id)를 쓰면 판정은 해결된다. 트리거 이관의 선행 조건 -->
- [ ] 기여 로그를 DB 트리거로 이관 (클라이언트 recordContribution 제거) <!-- 현재 화면마다 손으로 호출해 EntityFormModal·useReassignCategory(미분류 일괄 이관)·ImporterModal·글로벌 수정이 통째로 누락, 구분 변경은 created 2건 중복(NetworkForm:198 + useMoveEntity:266). 원장 트리거로 옮기면 누락·중복·원자성 동시 해소. 단계: (1) RLS 정합화 선행 → (2) 업로드 없는 워크스페이스(MANAGEMENT·FUND·OFFICE·ADMIN·PROGRAM)부터 트리거 + 클라이언트 호출 제거 → (3) NETWORKS·STARTUP은 대량 업로드가 source='upload'·batch_id를 클라이언트에서 남기고 있어 트리거와 반드시 중복되므로, 업로드 경로를 SECURITY DEFINER RPC로 감싸 원장 변경과 로그를 한 트랜잭션에 넣은 뒤 합류. 주의: 비활성 사유는 원장 컬럼이 아니라 entity_contributions.note에만 있어(20260707170000) 트리거가 복원할 수 없다 — 사유가 필요한 deactivated/merged는 RPC 경유가 정답 -->
