...
- [ ] 조직 관리 (N-depth 조직도 · 조직 레벨 · 인력 배치) <!-- '부서 관리'→'조직 관리' 개칭. UI 목업 완료(DB 미연동, 로컬 상태): parent_id 자기참조 N-depth 재귀 트리-테이블, 네이티브 드래그로 순서·소속(재부모화) 변경, 조직 레벨(노드별 태그 · 회사›본부›팀›파트 + α 자유 정의)→인사관리 컬럼 자동 파생(resolveByLevel)+반영 미리보기, 인력 배치 모달(실제 useEmployees/useDepartments 매핑), soft delete/영구삭제, 그리드 컬럼 정렬. 실 연동 대기: 마이그레이션(org_levels·org_versions·departments.level_id/sort_order/parent_id 쓰기 RLS) → 저장 mutation → EmployeeDirectory 컬럼 동적화. 확정: 회사도 조직 레벨(A안), 레벨=노드별 태그(B안) -->
