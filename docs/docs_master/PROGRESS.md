...
- [x] 7-17. 프로그램 상태 단계 이원화 — 제안 단계(제안/미선정 + 제안서 작성~발표 기간) / 운영 단계(준비/진행중/종료/취소 + 실제 행사 관리 기간) — [3_4_2](../docs_planning/3_4_2_ac_program_overview.md) <!-- programs에 proposal_start_date/proposal_end_date 신설(20260716150000, 기존 start/end_date=운영 기간으로 의미 확정, enum 변경 없음). ProgramFormModal을 단계 라디오+제안/운영 2섹션 카드(ProgramStageFields 신설)로 재구성 — 현재 단계의 상태 셀렉트만 활성, 기간은 상시 입력. 상세 기본데이터 카드 제안/운영 기간 병기, 목록 컬럼 운영 시작/종료일로 명명. tsc 통과 -->
