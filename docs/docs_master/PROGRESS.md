...
- [ ] 기여 로그 트리거 이관 2단계 — NETWORKS·STARTUP 원장 <!-- 현재 화면마다 손으로 호출해 EntityFormModal·useReassignCategory(미분류 일괄 이관)·ImporterModal·글로벌 수정이 통째로 누락, 구분 변경은 created 2건 중복(NetworkForm:198 + useMoveEntity:266). 원장 트리거로 옮기면 누락·중복·원자성 동시 해소(1단계에서 사업 3종은 이 방식으로 완료). 남은 난점은 대량 업로드다 — NETWORKS·STARTUP은 source='upload'·batch_id를 클라이언트가 남기고 있어 단순 트리거와 반드시 중복되므로, 업로드 경로를 SECURITY DEFINER RPC로 감싸 원장 변경과 로그를 한 트랜잭션에 넣은 뒤 합류시킨다. MANAGEMENT·FUND·OFFICE·ADMIN은 기여 기록 경로가 애초에 없어 대상 아님(ADMIN 병합 콘솔은 networks 원장을 쓰므로 이 단계에 포함). 주의: 비활성 사유는 원장 컬럼이 아니라 entity_contributions.note에만 있어(20260707170000) 트리거가 복원할 수 없다 — 사유가 필요한 deactivated/merged는 RPC 경유가 정답 -->
