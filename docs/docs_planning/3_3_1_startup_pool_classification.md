...
"누가 만들었나"와 "누가 관리하나"는 별개 축입니다. 아래 세 용어는 서로 겹치지 않으며, 코드·주석·화면 라벨에서 혼용하지 않습니다(특히 등록자를 담당자 자리에 폴백 표시하지 않습니다).

| 용어 | 정의 | 저장 위치 | 성격 |
| :--- | :--- | :--- | :--- |
| **등록자** | 이 레코드를 최초로 업로드/작성한 사람 | `startups.created_by` | 레코드마다 고정·불변, 모든 마스터에 존재 |
| **담당자** | 이 정보를 관리하는 주체 (아래 2모드) | 모드별 상이 | 관리 주체를 지칭 |
| ┗ 공동관리(비투자) | 특정인 없음. NETWORKS 쓰기 권한자 누구나 관리, 수정 이력은 기여자 로그로 누적 | `entity_contributions`(역할이 아닌 로그) | 발굴·보육·기타 |
| ┗ 특정 지정(투자) | 배정된 특정인(리드 1 + 지원 N)만 관리 | `startup_managers` | 투자기업 |
| **관리자** | 시스템 전역 우회 권한 | `current_app_role() = 'super_admin'` | 위 두 축과 직교하는 예외적 전역 권한 |

> [!NOTE]
> "관리자"라는 단어는 `super_admin`(예외적 전역 권한)에만 사용합니다. 공동관리하는 주체는 "담당자"이지 "관리자"가 아닙니다. 투자기업의 담당자(특정 지정)와 등록자는 서로 다를 수 있으며, 화면에서도 별개 컬럼으로 표기합니다.

### 4.1 권한 규칙

* **원칙**: 투자기업에만 지정 담당자가 존재하며, 지정 담당자와 관리자만 해당 기업 정보를 생성·수정·삭제할 수 있습니다. 담당자가 없는 구분(발굴·보육·기타)은 NETWORKS 쓰기 권한자 누구나 편집할 수 있습니다(공동관리).
...
* **등록자 vs 담당자 분리**: 등록자 컬럼은 전 구분 공통으로 `created_by`를 표시합니다. 담당자 컬럼도 전 구분 공통으로 노출하되, 비투자(발굴·보육·기타) 및 NETWORKS 8종은 `공동관리` 텍스트를, 투자기업은 지정 담당자(리드, 외 N)를 표시합니다. 등록자를 담당자 자리에 폴백 표시하지 않으며, 실제 기여자 명단은 상세 페이지에서 확인합니다.
