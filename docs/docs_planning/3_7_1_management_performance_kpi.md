...
* **포함**:
  ...
  * **KPI 세트(Set) 노코드 빌더** — 부서/개인 × 본부·계보 × 역할별 항목 묶음 설계·복제
  * 대상 배정 및 대상별 목표값(target) 오버라이드
  * 실적 입력 및 전자문서 증빙 연동
  * 점수 산출 엔진 및 세트 종합점수 집계
...
## 3. 핵심 사용자
* **KPI 설계자 (`super_admin`, management write)**: 지표·스코어링 룰·KPI 세트를 노코드 빌더로 설계·복제하고, 사이클과 정원·보정 정책을 운영합니다.
...
`성과·KPI 관리` 탭은 기존 `조직 관리` 탭과 형제 관계로 배치되며, 조직도 트리와 계보를 공유합니다. 구조는 **재사용 카탈로그(지표·룰) → 세트 빌더 → 대상 배정 → 실적/증빙 → 집계/평가**의 흐름입니다.
...
[성과·KPI 관리 탭]
 ...
 │    └── KPI 세트 (Set: 부서/개인·본부·역할별 항목 묶음 = 첨부문서 '한 장', 복제 지원)
 ├── 3. 배정 영역 (세트 → 조직 계보/임직원 배정 + 대상별 목표값 오버라이드)
 ├── 4. 실적 입력 (항목별 실적값/정성등급 + 전자문서 증빙 링크)
 └── 5. 집계·평가 대시보드
      ├── 세트 종합점수 (가중 합산, 조직도 트리 오버레이)
      ├── 상대평가 → 정원 배분 → 등급(S~E)
...
### 5.2 KPI 세트 빌더 + 실적/평가 대시보드
```
┌────────────────────────────────────────────────────────────────────────┐
│ [성과·KPI 관리]  사이클: [2026년 · OPEN ▼]                              │
├───────────────────────────────────────┬────────────────────────────────┤
│ ■ KPI 세트: AC본부 그룹 KPI            │ ■ 실적/점수: 스케일업그룹        │
│  scope: 부서 | 본부: AC | 역할: —      │  종합점수: 612 · 등급 A (2위)   │
│  [세트 복제] [항목 추가]               │ ───────────────────────────────  │
│  ┌──────────────────────────────────┐ │ 매출 KPI (BAND) 목표25억         │
...
* **세트 빌더**: 조직 관리의 `OrgVersionBar`/`OrgTreeEditor` UI 패턴을 재사용하며, 세트 복제로 전년도 기준을 빠르게 승계합니다.
...
* 하나의 지표는 여러 세트에서 서로 다른 룰·배점·목표로 재사용될 수 있습니다.
...
### 6.3 KPI 세트 빌더 (Set)
* **기능**: (측정 대상 × 적용 범위)별로 항목을 묶어 하나의 세트로 구성합니다. 세트 = 첨부문서의 "한 장"(예: `AC본부 그룹 KPI`, `신사업본부 개인 KPI(팀원)`).
* **스코프**: `scope_type`(`ORG` 부서 / `INDIVIDUAL` 개인), `division_lineage_id`(본부·조직 계보), `role_filter`(직책 — 팀원/그룹장/PM 등, 개인 세트의 역할별 상이 흡수).
* **항목**: 각 항목은 {지표 + 스코어링 룰 + 배점(max_score) + 기본 목표값}으로 구성합니다.
* **복제**: 기존 세트를 복제해 연도·본부 변형을 빠르게 만듭니다.

### 6.4 대상 배정 및 목표값 오버라이드
* **배정**: 세트를 실제 대상(조직 계보 또는 임직원)에 배정합니다.
* **목표값 오버라이드**: 상대수 지표(달성률)는 대상마다 목표(분모)가 다릅니다(예: 스케일업 25억, 밸류커넥트 20억). 배정별 `target_value`로 오버라이드하며, 절대수 지표는 목표 없이 값 자체를 룰에 투입합니다.
...
* **세트 종합점수**: 항목 점수를 배점 기준으로 합산해 대상의 종합점수를 산출하고, 조직도 트리에 오버레이합니다.
...
* **MI**: 그룹별 별도 인센티브 지표(총합 120점 초과 시 최상위 그룹 지급, 동률 시 공동 지급 등)를 별도 세트/룰로 표현합니다.
...
interface KpiSet {
  ...
  scope_type: 'ORG' | 'INDIVIDUAL';
  division_lineage_id: string | null;                      // 본부/조직 계보
  role_filter: string | null;                              // 개인 세트의 직책(팀원/그룹장/PM)
  status: 'DRAFT' | 'PUBLISHED' | 'ARCHIVED';
  deleted_at: string | null;
}

// 세트 내 항목 (지표 × 룰 × 배점)
interface KpiSetItem {
  id: string; set_id: string; metric_id: string; rule_id: string;
  max_score: number;                                       // 배점
...
interface KpiAssignment {
  id: string; set_id: string; cycle_id: string;
...
interface KpiTarget {
  id: string; assignment_id: string; set_item_id: string;
  target_value: number;
}
...
* **KPI 세트**(`KpiSet.status`)는 `DRAFT`(설계) → `PUBLISHED`(사이클 적용) → `ARCHIVED`(연도 종료 보관)의 전이를 가집니다.
* **실적/평가 확정**은 전자결재 승인과 매핑되어 `LOCKED` 처리됩니다.

---

## 9. 권한/RLS
* **설계(빌더)**: 지표·룰·세트·정책 편집은 KPI 설계자(`super_admin`, management write)만 가능합니다.
...
* **`fn_calc_set_score(p_assignment_id)`**: 배정 대상의 항목별 점수를 산출·합산해 종합점수를 반환합니다.
...
* **조직 관리**: KPI 세트·배정의 대상은 조직 계보(`lineage_id`)를 원천으로 하며, 개편 시에도 연속됩니다.
* **HRM/HRD**: 개인 세트의 `role_filter`는 인사 직책(그룹장/팀원/PM)과 매핑되고, 사이클 시점 소속은 `dept_members`로 해석합니다. 발령 이력(`hr_assignments`)·프로필(`hr_profiles`)과 연동합니다.
...
* **ADMIN 감사 로그**: 룰·세트 변경, 실적 확정, 등급·지급률 확정 등 민감 액션은 ADMIN 감사 로그에 실행자·대상을 적재합니다.

---

## 13. 예외/오류/운영 리스크
* **사이클 진행 중 조직 개편 발생(★ 핵심 리스크)**: 배정을 계보(`subject_lineage_id`)에 귀속시켜, 개편으로 부서명·상위가 바뀌어도 목표·실적·결과가 동일 조직으로 이어집니다. 조직이 분할/통합되어 계보가 갈라지면 담당자가 배정 이관을 명시적으로 결정하도록 안내합니다.
...
1. 관리자가 노코드 빌더로 지표·스코어링 룰·KPI 세트를 만들고, 세트를 복제해 본부/연도 변형을 생성할 수 있는가?
...
4. 배정·실적·결과가 부서 시점 id가 아닌 계보(`subject_lineage_id`)에 저장되어, 조직 개편 후에도 연속 조회되는가?
5. 종합점수 상대평가로 정원 배분 등급(S~E)이 산출되고, 조건부 보정과 지급률이 정책대로 반영되는가?
6. 모든 실적에 전자문서 증빙이 연결되며, 미증빙 시 확정이 차단되는가?
7. 외부 게스트 계정으로 성과 API 호출 시 RLS에 의해 정상 차단되는가?
...
6. **계보 연속성 테스트**: 사이클 진행 중 조직 개편(부서명·상위 변경) 후에도 개편 전 실적·결과가 동일 계보로 조회되는지 확인합니다.
7. **RLS 테스트**: 외부 게스트 및 타 조직 임직원 토큰으로 실적/결과 조회·쓰기를 시도해 계보 범위 밖 접근이 차단되는지 확인합니다.
