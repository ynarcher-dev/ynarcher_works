...
> [!NOTE]
> **모집 공개 링크는 모듈 링크 공유의 첫 사례이며, 2026-09-02에 그 축으로 이관되었습니다.** 로그인 없이 여는 주소·공개 상태·공개 기간은 이제 `program_module_public_links`가 소유하고, 규칙의 정본은 [3_4_15](./3_4_15_ac_public_links.md)입니다.
>
> **토큰 값을 보존했으므로 이미 배포된 `/apply/:token` 주소는 바뀌지 않았습니다.** `application_forms`의 `public_token`·`public_status`·`open_at`·`close_at`은 동결되어 더는 읽지도 쓰지도 않습니다 — 미러를 두면 언젠가 어긋나고, 그때 어느 쪽이 진짜인지 판정할 근거가 없습니다. 랜딩 콘텐츠·신청서 필드·동의 문구는 모집 고유의 것이라 계속 `application_forms`가 소유하며, 저장 창구도 `set_application_form` 하나 그대로입니다(담기는 원장이 달라졌을 뿐 화면은 알 필요가 없습니다).
>
> 모집은 이 축에서 **유일하게 쓰기(제출)가 있는** 사례이므로 개인정보 수집·이용 동의, 중복 제출 처리, 자동 제출 방어가 추가로 따라붙습니다. 이관으로 공개 게이트가 **모듈·사업 생존과 ADMIN 상한을 함께 보게 되어**, 담당자가 모듈을 껐는데도 신청서는 열려 있던 구멍이 닫혔습니다.
