...
> [!NOTE]
> **모집 공개 링크는 모듈 링크 공유의 첫 사례입니다**(2026-09-02). 로그인 없이 여는 주소·공개 상태·공개 기간이라는 축은 모집만의 것이 아니라 모듈 공통 기능으로 승격되었으며, 규칙의 정본은 [3_4_15](./3_4_15_ac_public_links.md)입니다. 모집은 그 축에서 **유일하게 쓰기(제출)가 있는** 사례이므로 개인정보 수집·이용 동의, 중복 제출 처리, 자동 제출 방어가 추가로 따라붙고, 랜딩 콘텐츠·신청서 필드·동의 문구는 계속 `application_forms`가 소유합니다. 토큰·공개 상태·기간의 링크 원장 이관은 2단계로 진행하되 **토큰 값을 보존**하므로 이미 배포된 `/apply/:token` 주소는 바뀌지 않습니다.
