...
* **프로그램 상태(단계 이원화)**: 프로그램 대표 상태는 제안 단계와 운영 단계로 나뉘며, 단계별로 별도의 기간 필드를 가집니다. 등록/편집 폼도 두 섹션으로 분리해 현재 단계에 해당하는 상태만 선택할 수 있습니다.
  ...
  * **기간 제약**: 제안 기간과 운영 기간은 서로 겹칠 수 없습니다(양쪽 모두 시작·종료일이 입력된 경우 등록/편집 시 검증). 운영 모듈의 기간(`program_modules.settings.start_date`~`end_date`)은 반드시 제안 기간 또는 운영 기간 중 한 구간 안에 완전히 포함되어야 하며, 모듈 설정 저장 시 검증합니다.
