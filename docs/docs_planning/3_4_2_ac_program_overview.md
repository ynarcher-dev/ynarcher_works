...
* **인스턴스 세팅**: 각 인스턴스는 `상태 · 공유 범위(visibility) · 시작일 · 종료일 · 담당자(다중) · 설명(메모)`을 개별 보유합니다.
  ...
  * **선택지의 상한은 ADMIN이 정합니다**(2026-09-02): 공유 범위를 WORKS+GUEST까지 올릴 수 있는지, 링크 공유 칸이 서는지는 템플릿 원장([3_2_1](./3_2_1_admin_module_registry.md)의 `module_templates`)이 답합니다. 담당자는 그 상한 **안에서** 고르며, 상한을 넘는 저장은 `set_program_module`이 거부합니다. 링크 공유 스위치는 상한이 열려 있어도 **ADMIN만** 조작합니다(담당자에게는 읽기 전용).
