...
* **인스턴스 삭제(하드 딜리트, 2026-09-02 신설)**: 카드의 '삭제(휴지통)'는 인스턴스를 원장에서 물리적으로 제거합니다. 끄기와는 **다른 축**입니다 — 끄기는 되돌릴 수 있는 운영 중단이고, 삭제는 잘못 만든 배치물을 없던 것으로 되돌리는 정리입니다. 끄기만 있던 동안 오생성 인스턴스가 원장에 계속 남아 모듈명 유일 제약과 `OUTCOMES` 단일 제약을 점유했습니다. 세 관문을 모두 통과해야 실행됩니다.
  ...
  * **빈 그릇만 지웁니다**: 모듈에 딸린 업무 기록(지원서·평가 제출·선정 결과·멘토링 기록·글·첨부 등)이 한 건이라도 남아 있으면 서버가 삭제를 거부하고, 무엇이 몇 건 남았는지 삭제창에서 먼저 보여 줍니다. 이 경계 덕분에 물리 삭제가 열려도 **기록을 지우는 경로는 생기지 않습니다**. 이미 지운 행(`deleted_at`)은 잔존으로 세지 않습니다 — 화면에서 비어 보이는 모듈이 영영 안 지워지면 안 되기 때문입니다.
  * **양식·항목 정의는 기록이 아니라 설정입니다**(2026-09-02 보완): 모집 폼(`application_forms` — 모듈과 1:1)·폼 항목·평가표·평가 항목은 차단 사유에서 제외합니다. 가르는 기준은 *무엇을 받을지·무엇을 잴지 정해 둔 것인가, 사람이 제출·기록한 것인가*입니다. 폼은 모듈을 세울 때 함께 만들어지는 모듈의 일부라 담당자 배정·링크 설정과 같은 자리이며, 이를 기록으로 세는 동안에는 **모집 모듈이 설정을 마치는 순간 영영 지워지지 않았습니다**. 설정 표는 사유에서 빠질 뿐 **그 아래 제출 기록은 그대로 셉니다** — 폼을 세지 않는다고 폼에 달린 지원서까지 놓치면 정작 막아야 할 것을 놓칩니다.
    * 이 목록만 손으로 나열하는 이유는 방향이 안전하기 때문입니다. 목록에서 빠진 표는 계속 차단하므로 빠뜨렸을 때의 결과는 '못 지운다'(불편)이지 '지워진다'(유실)가 아닙니다. 반대로 내용물 쪽을 나열했다면 빠뜨린 표가 곧 유실 경로가 되므로, 그쪽은 카탈로그가 답합니다.
