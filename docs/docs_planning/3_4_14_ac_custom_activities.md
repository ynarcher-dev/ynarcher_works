...
* 이동할 탭 키는 화면이 아니라 `MODULE_META[type].tab`이 정하며, 목록·칸반·간트 세 뷰가 같은 판정을 공유합니다.
...
기본 템플릿 3종은 자체 상태를 갖지 않고 모듈 인스턴스의 상태(`module_status`: `DRAFT`/`OPEN`/`CLOSED`/`CANCELLED`)와 공유 범위(`module_visibility`: `INTERNAL_ONLY`/`GUEST_ONLY` — 2026-09-02 2값으로 정리)를 따릅니다. 배정 방식(`participation_mode`)은 셋 모두 `ADMIN_ONLY` 고정입니다 — 운영자가 직접 남기는 기록이라 배정할 대상이 없습니다. 링크 공유를 켜면 그 상태가 하나 더 붙지만 모듈 상태와는 다른 축이며, 자체 상태 3단은 [3_4_15 §8](./3_4_15_ac_public_links.md)이 소유합니다.
...
기본 템플릿 3종은 두 경로로 바깥에 섭니다. **둘은 서로를 전제하지 않으며 각각 독립적으로 켜고 끕니다.**

* **로그인한 참여기업(GUEST 포털)**: 공유 범위를 WORKS+GUEST(`GUEST_ONLY`)로 올리면 사이드바에 한 줄로 서고, WORKS와 같은 화면 구성을 편집 권한만 뺀 채 공유합니다(2026-09-01). 다만 메뉴가 서는 것과 그 안이 열리는 것은 다른 물음이라, 준비(`DRAFT`)·취소 단계의 글·링크·파일은 열리지 않습니다.
* **로그인 없는 외부(모듈 공개 링크)**: 셋 모두 링크 공유를 켤 수 있는 **읽기 전용** 템플릿입니다 — 공고문·주소 모음·배포 파일은 누가 읽든 같은 화면이라 상대가 누구인지 알 필요가 없기 때문입니다. 발급·기간·죽는 조건은 [3_4_15](./3_4_15_ac_public_links.md)가 소유합니다. 파일첨부에 링크를 켤 때는 그 파일이 사업 자료 관리에도 같은 행으로 서 있다는 사실과 나갈 파일 목록을 확인창이 함께 말합니다.
