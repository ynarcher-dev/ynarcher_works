...
* 사업구분(공공·민간·매출·기타)은 2026-08-03부터 사이드바 세분화 메뉴가 아니라 목록 필터의 축입니다. 미지정 건은 그 필터의 `미지정` 선택지로 고릅니다.
* 목록은 사업명·생성자 검색, 상태·사업구분·담당 부서·시작일 필터, 서버 페이지네이션, 다중 선택을 제공합니다. 생성자는 권한 축이 아니므로 목록에서 숨기고 담당 부서·담당자가 현재 관리 주체를 답합니다.
