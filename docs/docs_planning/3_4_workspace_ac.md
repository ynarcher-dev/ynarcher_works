...
* `전체 프로젝트(?scope=all)`는 같은 목록을 담당 스코프 없이 넓힌 범위이며, 목록 상단의 범위 토글로 오갑니다. 옛 링크(`?tab=mine`·`?tab=all`·`?tab=dashboard`·사업구분별 탭)로 들어오면 이 화면으로 주소까지 바로잡습니다. [3_4_1_ac_dashboard.md](./3_4_1_ac_dashboard.md)의 종합 대시보드는 현재 렌더링이 아니라 목표 기획입니다.
