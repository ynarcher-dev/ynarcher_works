...
* `/ac`의 기본 진입은 `?tab=mine`이며 `내 사업 관리` 목록을 엽니다.
* `전체 사업(?tab=dashboard)`은 전면 재설계를 위해 제목만 남긴 골격입니다. [3_4_1_ac_dashboard.md](./3_4_1_ac_dashboard.md)는 현재 렌더링이 아니라 목표 기획입니다.
