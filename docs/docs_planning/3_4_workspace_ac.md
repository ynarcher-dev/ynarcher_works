...
* 사업은 산업 태그(`industry_tags`, 최대 3개)를 가집니다. "이 사업이 어느 산업군의 스타트업을 발굴·대상으로 하는가"에 답하는 축이며, 스타트업 원장(`startups.industries`)과 같은 태그 원장을 읽습니다. 등록·편집 모달과 대용량 업로드의 `산업` 열에서 지정합니다.
* 목록은 사업명·생성자 검색, 상태·사업구분·담당 부서·시작일 필터, 서버 페이지네이션, 다중 선택을 제공합니다. 목록 컬럼은 사업명·코드·카테고리·담당 부서·산업·상태·운영 시작일·운영 종료일·담당자이며, 종전의 `설명` 열은 산업이 대신합니다(설명은 상세 헤더가 답합니다). 생성자는 권한 축이 아니므로 목록에서 숨기고 담당 부서·담당자가 현재 관리 주체를 답합니다.
* 상세는 공용 사업 정보 카드, 모듈 보드, 일정·전자결재·회의록·자료·코멘트·변동 이력 패널을 사용합니다. 생성자는 상세 기본 정보에 기록값으로만 표시합니다.
* 모듈 타입 9종(모집, 서면/대면평가, OT, 멘토링, 비즈니스 매칭, 데모데이, 성과, 커스텀 활동)은 화면에 연결되어 있으나 각 세부 문서의 모든 자동화·외부 연동이 완료되었다는 뜻은 아닙니다.
