...
* `/ac`의 사이드바는 `프로젝트` 한 줄이며(2026-09-05 통합) 기본 진입은 `내 프로젝트` 범위입니다.
* `전체 프로젝트`가 기본 범위이고(2026-09-07 전환) `내 프로젝트(?scope=mine)`는 같은 목록을 담당 스코프로 좁힌 범위이며, 목록 상단의 범위 토글로 오갑니다. 옛 링크(`?tab=mine`·`?tab=all`·`?tab=dashboard`·사업구분별 탭)로 들어오면 이 화면으로 주소까지 바로잡습니다. [3_4_1_ac_dashboard.md](./3_4_1_ac_dashboard.md)의 종합 대시보드는 현재 렌더링이 아니라 목표 기획입니다.
* 사이드바·페이지 제목 라벨은 2026-08-20부터 M&A·PROJECT와 같은 말을 쓰며(종전 `내 사업 관리`/`전체 사업`), 2026-09-05에 그 한 쌍이 메뉴 한 줄 `프로젝트`(상수 `PROGRAM_LIST_LABEL`)와 목록 범위 토글로 합쳐졌습니다. 세 워크스페이스가 같은 원장 구조와 같은 화면을 쓰므로 메뉴 이름도 하나로 두며, 도메인 명칭 '사업'은 목록 안쪽 문구(검색 자리표시자·`사업 등록` 버튼·업로드 템플릿)와 범위 토글의 명사가 계속 씁니다.
...
* 목록은 프로젝트명·생성자 검색, 상태·프로젝트 구분·담당 부서·시작일 필터, 서버 페이지네이션, 다중 선택을 제공합니다. 목록 컬럼은 프로젝트명·코드·카테고리·담당 부서·분야·상태·운영 시작일·운영 종료일·담당자이며, 종전의 `설명` 열은 분야가 대신합니다(설명은 상세 헤더가 답합니다). 생성자는 권한 축이 아니므로 목록에서 숨기고 담당 부서·담당자가 현재 관리 주체를 답합니다.
