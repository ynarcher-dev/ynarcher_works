...
* 공공·민간·매출·기타 사업은 같은 목록에 카테고리 프리필터만 적용합니다. 기타 사업은 카테고리 미지정 건도 포함합니다.
* 목록은 사업명·생성자 검색, 상태·시작일 필터, 서버 페이지네이션, 다중 선택을 제공합니다. 생성자는 권한 축이 아니므로 목록에서 숨기고 담당 부서·담당자가 현재 관리 주체를 답합니다.
