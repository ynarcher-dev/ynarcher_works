...
OFFICE의 게시 기능을 **게시판(POST)** 과 **자료실(ARCHIVE)** 두 종류로 확정하고, 공지사항을 별도 게시판이 아닌 **게시글 플래그 기반의 조회 뷰**로 정의합니다. 게시판과 자료실 모두 운영 중 필요할 때마다 생성·확장할 수 있는 레지스트리 구조를 사용합니다.

> [!NOTE]
> **구현 상태(2026-07-31)**: `boards`·`board_posts`·`board_comments` 원장과 RLS, 게시글 작성/상세/댓글/조회수, 자료실 1행 1파일 다운로드, 전체 공지 뷰, ADMIN 게시판 관리가 연결되어 있습니다. OFFICE 사이드바는 `공지사항`을 고정 메뉴로 두고 `게시판`·`자료실`은 각각 우측 플라이아웃에서 실제 레지스트리를 선택합니다.
...
* **표에 남는 조작은 다운로드 하나입니다**(2026-08-25): 수정·비활성화는 행을 눌러 여는 **자료 모달**(`ArchiveDetailModal`)이 갖고, 목록의 `관리` 열은 내립니다(`showManageColumn={false}`). 두 버튼이 행마다 반복되면 정작 자료명·설명보다 먼저 읽히고, 되돌리기 어려운 비활성화가 목록을 훑는 손이 지나는 자리에 서게 됩니다 — 어느 자료에 대고 하는 일인지 확인하지 않고 누를 수 있는 버튼은 그 자체가 위험합니다. 다운로드만 표에 남기는 이유는 자료실을 여는 사람이 가장 자주 하는 일이면서 되돌릴 것이 없는 조회 행위이기 때문입니다. 삭제·수정을 상세로 옮긴 STARTUP·PROGRAM 목록과 같은 판단입니다.
