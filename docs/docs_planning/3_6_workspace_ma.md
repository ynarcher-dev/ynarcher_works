...
* **대시보드**: 딜 상태 요약 타일. 진행 중 건수와 전체 건수를 노출합니다.
* **내 프로젝트**: 로그인 사용자가 담당자 또는 등록자인 딜만 필터링합니다.
...
화면은 라우트가 아니라 `?tab=` 쿼리스트링으로 전환합니다. 카테고리 항목의 `tab` 값은 소문자 사업구분 값(예: `pe_fund`)이며, `내 프로젝트`와 세분화 항목은 동일 컴포넌트에 `scope`·`category` 값만 달리 전달합니다.

'전체' 항목은 두지 않습니다. 대신 `기타 프로젝트`가 사업구분 미지정(`category is null`) 건까지 함께 담아, 어느 메뉴에서도 보이지 않는 건이 생기지 않도록 합니다.
