...
* **사업구분 세분화**: 이어서 `Sell`, `Buy`, `PE Fund`, `기타` 항목이 나열되며, 각각 해당 사업구분의 딜만 조회합니다.

화면은 라우트가 아니라 `?tab=` 쿼리스트링으로 전환합니다. 카테고리 항목의 `tab` 값은 소문자 사업구분 값(예: `pe_fund`)이며, `내 딜`과 `전체 딜`·세분화 항목은 동일 컴포넌트에 `scope`·`category` 값만 달리 전달합니다.
