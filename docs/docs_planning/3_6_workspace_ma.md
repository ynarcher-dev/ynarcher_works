...
* **내 프로젝트**(기본): 로그인 사용자가 담당자 또는 생성자인 딜만 조회합니다.
* **전체 프로젝트**(기본): 같은 목록을 담당 스코프 없이 넓혀 씁니다. 좁힌 범위는 `?scope=mine`입니다(2026-09-07 기본 전환).

> [!NOTE]
> M&A의 관리 대상은 딜이지만 화면 표기는 다른 사업 워크스페이스와 맞추어 '프로젝트'로 부릅니다(`MNA_WORKSPACE.entityNoun`). 2026-08-20에 메뉴 라벨도 같은 이유로 통일했습니다 — 종전 `내 딜 관리`/`전체 딜`을 AC·PROJECT와 같은 `내 프로젝트`/`전체 프로젝트`로 바꾸었고, 라벨은 `config/navigation.ts`의 `PROGRAM_LIST_LABEL`(메뉴 한 줄)과 범위 토글의 명사가 함께 답하며, 2026-09-05 통합 이후 메뉴 라벨의 단일 원천은 그 상수 하나입니다.

화면은 라우트가 아니라 `?scope=` 쿼리스트링으로 전환하며, 두 범위는 동일 컴포넌트에 `scope` 값만 달리 전달합니다. 옛 `?tab=` 주소는 이 화면으로 흡수합니다.
...
* **게스트는 딜 단위로만 열립니다(2026-09-08 개정)**: 종전에는 *"외부 게스트용 정책을 부여하지 않아 Default Deny로 차단"* 이었으나, 매도자에게 요청자료를 받고 인수후보에게 선별 자료를 여는 것이 이 워크스페이스의 실제 업무이므로 창구를 엽니다. 여는 단위는 워크스페이스가 아니라 **딜 하나**이며(`app.guest_program_ids()`가 `ma_programs`를 이미 판정합니다), 인수후보에게 보이는 자료는 담당자가 데이터룸 모듈에 명시적으로 넣은 것뿐입니다 — 기본은 여전히 닫힘입니다. 계정·창구·선별 규칙의 정본은 [3_9_2](./3_9_2_external_portal_expansion.md)입니다.
