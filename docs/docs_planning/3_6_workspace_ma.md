...
* **내 딜 관리**: 사이드바 첫 항목이자 탭 미지정 시 기본 화면입니다. 로그인 사용자가 담당자 또는 생성자인 딜만 조회합니다.
* **전체 딜**: `?tab=dashboard`. 2026-07-31 현재 전면 재설계를 위해 제목만 남긴 골격이며 상태 요약 타일은 렌더링하지 않습니다.
* **사업구분 세분화**: 이어서 `Sell 프로젝트`, `Buy 프로젝트`, `PE 프로젝트`, `기타 프로젝트` 항목이 나열되며, 각각 해당 사업구분의 딜만 조회합니다.

> [!NOTE]
> M&A의 관리 대상은 딜이지만 화면 표기는 다른 사업 워크스페이스와 맞추어 '프로젝트'로 부릅니다(`MNA_WORKSPACE.entityNoun`).

화면은 라우트가 아니라 `?tab=` 쿼리스트링으로 전환합니다. 카테고리 항목의 `tab` 값은 소문자 사업구분 값(예: `pe_fund`)이며, `내 딜 관리`와 세분화 항목은 동일 컴포넌트에 `scope`·`category` 값만 달리 전달합니다.
...
M&A는 AC(공공/민간/매출/기타)와 달리 다음 4분류를 사용합니다.
