...
* **거래상대 원장(`ma_sellers`·`ma_buyers`)의 중복 기준(2026-09-11)**: 정본은 [3_3_8](./3_3_8_ledger_identity_dedup.md)입니다. 스타트업 원장에 연결된 행은 스타트업당 살아있는 행 1건이고 사업자등록번호를 갖지 않으며(번호는 스타트업이 갖습니다), 미연결 행만 자기 번호(`biz_reg_no`)를 확실한 키로 갖습니다. 미연결 행이 스타트업 원장의 기업과 같으면(번호 또는 이름·이메일·전화 중 두 칸) 저장을 막고 연결을 요구합니다. 반대 방향(스타트업 등록 시 M&A 조회)은 매각 검토 사실이 새지 않도록 묻지 않습니다.
