...
* **전체 딜**: `?tab=all`. `내 딜 관리`와 같은 목록 화면을 담당 스코프 없이 넓혀 씁니다.
...
> [!NOTE]
> 2026-08-03에 사업구분별 세분화 항목(`Sell 프로젝트`·`Buy 프로젝트`·`PE 프로젝트`·`기타 프로젝트`)을 내렸습니다. 분류를 메뉴로 두면 그것이 '어디에 있는가'가 되어 상태·부서 같은 다른 축과 함께 걸 수 없고, 분류를 늘릴 때마다 사이드바가 길어집니다. 사업구분은 목록 필터의 축 하나가 되었고, 종전에 `기타 프로젝트`가 겸하던 미분류(`category is null`) 사각지대 방어는 구분 무관 전부를 보여주는 `전체 딜`과 필터의 `미지정` 선택지가 대신합니다. 옛 링크(`?tab=sell`, 구 `?tab=dashboard`)로 들어오면 `전체 딜`로 주소까지 바로잡습니다.
