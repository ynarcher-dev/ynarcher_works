...
*   **[1] HUB (허브)**
    *   **통합 검색 포털**: 전사 데이터 통합 조회 (`Global Search Portal`)
    *   **통합 일정 캘린더**: 부서 간 일정 오버레이 뷰 (`Global overlay Calendar`)
    *   **전문가 만족도 분석 보드**: 외부 전문가 만족도 랭킹 및 피드백 (`Expert Satisfaction Board`)
    *   **임직원 프로필 조회**: 전사 구성원 디렉토리 (`Staff Directory`)
*   **[2] NETWORKS (네트워크)**
