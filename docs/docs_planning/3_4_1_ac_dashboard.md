...
> [!IMPORTANT]
> **구현 상태(2026-07-31)**: 이 문서는 대시보드 **재설계 목표안**입니다. 현재 `/ac?tab=dashboard`의 `ProgramDashboardTab`은 콘텐츠를 렌더링하지 않으며 `전체 사업` 제목만 표시합니다. 실제 사용 화면의 기본 진입은 `?tab=mine`의 내 사업 목록이고, 공공·민간·매출·기타 카테고리 목록과 사업 상세는 사용 가능합니다.
...
* **OFFICE 일정**: 대시보드의 프로그램 기간(`start_date` ~ `end_date`) 및 중요 심사/행사 일정은 전사 `OFFICE` 캘린더와 실시간 테이블 트리거를 통해 동기화됩니다.
