...
* **전체 프로젝트**: `?tab=dashboard`. 2026-07-31 현재 전면 재설계를 위해 제목만 남긴 골격이며 상태 요약 타일은 렌더링하지 않습니다.
* **사업구분 세분화**: 이어서 `글로벌 프로젝트`, `신사업 프로젝트`, `기타 프로젝트` 항목이 나열되며, 각각 해당 사업구분의 프로젝트만 조회합니다.

화면은 `?tab=` 쿼리스트링으로 전환합니다. 카테고리 항목의 `tab` 값은 소문자 사업구분 값(예: `new_biz`)이며, `내 프로젝트 관리`와 세분화 항목은 동일 컴포넌트에 `scope`·`category` 값만 달리 전달합니다.

전체 **목록** 항목은 두지 않고 전체 **대시보드**만 둡니다. 대신 `기타 프로젝트`가 사업구분 미지정(`category is null`) 건까지 함께 담아, 어느 카테고리 메뉴에서도 보이지 않는 건이 생기지 않도록 합니다. 구 `?tab=all`은 기존 북마크와 상세 뒤로가기 호환을 위해 화면에서 계속 처리합니다.
