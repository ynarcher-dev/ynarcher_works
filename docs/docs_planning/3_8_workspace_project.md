...
세 화면은 `?tab=dashboard|mine|all` 쿼리스트링으로 전환하며, `내 프로젝트`와 `전체 프로젝트`는 동일 컴포넌트에 `scope` 값만 달리 전달합니다.
...
PROJECT는 현재 **`ETC`(기타) 단일 분류**만 운용합니다. 구 `project_type` 분류(글로벌/신사업/일반)는 폐지되었습니다.
