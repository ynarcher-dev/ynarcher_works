...
사이드바는 `프로젝트` 한 줄이며(2026-09-05 통합) 범위는 목록 상단의 토글이 정합니다.

* **내 프로젝트**(기본): 로그인 사용자가 담당자 또는 생성자인 프로젝트만 조회합니다.
* **전체 프로젝트**(기본): 같은 목록을 담당 스코프 없이 넓혀 씁니다. 좁힌 범위는 `?scope=mine`입니다(2026-09-07 기본 전환).

화면은 `?scope=` 쿼리스트링으로 전환하며, 두 범위는 동일 컴포넌트에 `scope` 값만 달리 전달합니다. 옛 `?tab=` 주소는 이 화면으로 흡수합니다.

> [!NOTE]
> 2026-08-03에 사업구분별 세분화 항목(`글로벌 프로젝트`·`신사업 프로젝트`·`기타 프로젝트`)을 내렸습니다. 분류를 메뉴로 두면 그것이 '어디에 있는가'가 되어 상태·부서 같은 다른 축과 함께 걸 수 없고, 분류를 늘릴 때마다 사이드바가 길어집니다. 사업구분은 목록 필터의 축 하나가 되었고, 종전에 `기타 프로젝트`가 겸하던 미분류(`category is null`) 사각지대 방어는 구분 무관 전부를 보여주는 `전체 프로젝트`와 필터의 `미지정` 선택지가 대신합니다. 옛 링크(`?tab=new_biz`, 구 `?tab=dashboard`)로 들어오면 `전체 프로젝트`로 주소까지 바로잡습니다.

### 1.2 리스트뷰

AC 사업 목록과 동일한 컬럼 구성(프로젝트명 → 코드 → 사업구분 → 담당 부서 → 설명 → 상태 → 시작일 → 종료일 → 담당자)을 사용하며, 사업명·생성자 검색, 서버 페이지네이션, 상태/시작일 필터, 다중 선택 동작이 동일합니다. 생성자는 목록에서 숨기고 상세 기본 정보에만 표시하며, 소프트 삭제는 상세에서 수행합니다.

* **사업코드**: 등록 시 6자리 영숫자 난수가 DB 트리거로 자동 부여됩니다.
* **상태 수명주기(운영 4단계)**: PROJECT는 AC와 달리 제안 단계를 운용하지 않습니다. 프로젝트는 착수 결정이 곧 시작이므로 `DRAFT`(준비) -> `OPERATING`(진행중) -> `FINISHED`(종료), 중단 시 `CANCELLED`(취소) 네 단계만 사용합니다. 등록/편집 폼에는 단계 라디오와 제안 블록이 없고 상태·기간이 한 평면에 놓이며, 목록 상단 진행 현황 프로세스 뷰도 이 네 칸만 그립니다. 운용 여부는 `ProgramWorkspaceConfig.hasProposalStage`(false)가 단일 원천이고, `project_programs`에는 제안 상태(`PROPOSED`/`SELECTED`/`NOT_SELECTED`) 저장을 막는 CHECK 제약이 걸려 있습니다(`20260803120000_program_stage_scope.sql`). 기존 제안 상태 행은 같은 마이그레이션이 시도·선정 -> `DRAFT`, 미선정 -> `CANCELLED`로 환산했습니다.
...
* **모듈 템플릿**: PROJECT는 **기본 템플릿 3종(글쓰기 `POST`·URL첨부 `LINK`·파일첨부 `FILE`)만** 운용합니다. AC 정형 운영 모듈은 노출하지 않으며, 모듈 추가 모달에서 '운영 템플릿' 섹션 자체가 렌더되지 않습니다.
