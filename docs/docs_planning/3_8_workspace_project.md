...
* **내 프로젝트**: 사이드바 첫 항목이자 탭 미지정 시 기본 화면입니다. 로그인 사용자가 담당자 또는 생성자인 프로젝트만 조회합니다.
* **전체 프로젝트**: `?tab=all`. `내 프로젝트`와 같은 목록 화면을 담당 스코프 없이 넓혀 씁니다.

화면은 `?tab=` 쿼리스트링으로 전환하며, 두 항목은 동일 컴포넌트에 `scope` 값만 달리 전달합니다.
...
AC 사업 상세와 **완전히 동일한 화면**입니다. 좌측 2/3에 기본 정보 카드와 운영 모듈 보드(칸반/간트 2뷰, 기본 간트), 우측 1/3에 일정 카드·관련 전자결재·자료·피드백·변경 이력을 배치합니다.
