#!/usr/bin/env node
// 재생성 엔트리포인트 — 결정 매니페스트에서 pgTAP 기대 행렬 부분을 찍어 내고, 손으로 쓴
// 시나리오 단언과 합쳐 supabase/tests/authorization_inventory_test.sql을 만듭니다.
// 산출된 .sql이 정본입니다. 사용: node scripts/security/_gen_test.mjs
...
// (표, 권한문자열) — 권한이 하나도 없는 표는 빈 문자열로 함께 적습니다. 그래야 "줘서는 안 되는데
// 준" 경우도 같은 비교 한 번에 걸립니다.
function matrixRows(pick) {
  return d.decisions
    .map((x) => `    ('${x.table}', '${ROW.filter((o) => pick(x).includes(o)).join(',')}')`)
    .join(',\n')
}

const AUTH_DELETE = d.decisions.filter((x) => x.expected.authenticated.includes('DELETE')).map((x) => x.table).sort()
const nAuth = d.decisions.filter((x) => x.proposed.authenticated.length).length
const nSvc = d.decisions.filter((x) => x.proposed.service_role.length).length
...
fs.writeFileSync(path.join(ROOT, 'supabase/tests/authorization_inventory_test.sql'), sql, 'utf8')
console.log('test written. expected rows:', { authenticated: nAuth, service_role: nSvc })
