 ...
 * 같아도 조립이 각자면 어느 화면에서는 종류가 서고 어느 화면에서는 자격만 서는 일이 생긴다 —
 * 실제로 선택 화면에는 종류(프로젝트/M&A/FUND)가 아예 없었고, 같은 회사가 사업에도 조합에도
 * 걸리면 목록의 두 줄이 제목만으로는 갈리지 않았다. 조립 규칙을 한곳에 두고 화면은 그린다.
 */

/** 태그 한 칸. 모두 중립 톤이다 — 아래 주석 참조. */
export interface ContextTag {
  /** 같은 목록 안에서 key로 쓸 수 있는 값(라벨이 곧 종류·자격의 이름이라 겹치지 않는다). */
  key: string
  label: string
}

/**
 * 이 맥락을 설명하는 태그 — **종류 다음 자격** 순서로 최대 두 칸이다.
 *
 * 색은 쓰지 않는다(전부 중립). 색이 답하는 것은 상태(진행중·종료·취소)이고, 종류와 자격은
 * 상태가 아니라 분류다 — 분류마다 색을 주면 같은 목록에서 '진행중'과 'FUND'가 같은 무게로
...
export function contextTags(
  ...
  persona: GuestPersona | null | undefined,
): ContextTag[] {
  ...
  if (persona) tags.push({ key: `persona:${persona}`, label: PERSONA_LABEL[persona] })
  return tags
}
