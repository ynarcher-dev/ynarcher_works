/**
 * 게스트 목록 화면(QNA·공지사항)의 검색·페이징 규칙.
 *
 * 두 화면이 같은 규칙을 쓰므로 한곳에 둔다 — 화면마다 검색 대상 필드나 페이지 크기가
 * 갈리면 같은 모양의 목록이 서로 다르게 반응한다. 서버 검색이 아니라 **화면 안 걸러내기**다:
 * 게스트에게 돌아오는 행은 RLS가 이미 자기 사업·본인 것으로 좁혀 둔 소량이라, 왕복을
 * 늘리는 대신 받은 목록에서 거른다.
 */

/** 한 쪽에 보일 건수. 카드 안 보조 목록이라 미니 페이저와 함께 짧게 끊는다. */
...
/**
 * 제목·본문에 검색어가 든 행만. 대소문자를 가리지 않으며, 본문이 비어 있으면 제목만 본다.
 * `extra`는 화면이 함께 검색 대상으로 삼을 값(예: QNA의 답변 본문)이다.
 */
