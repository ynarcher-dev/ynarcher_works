import { Card, Field } from '@ynarcher/ui'
 ...
 * 입력 규격은 폼 공용 `Field`+`PasswordInput`을 쓰되 높이만 GUEST 터치 하한(48px)으로 올린다.
 */
export function PasswordChangeCard() {
  ...
  const ruleBroken = next.length > 0 && !passwordRuleOk(next)
  ...
  return (
    <Card title="비밀번호 변경" subtitle={PASSWORD_RULE_TEXT}>
      ...
      >
        ...
        <Field label="새 비밀번호" error={ruleBroken ? PASSWORD_RULE_TEXT : undefined}>
          <PasswordInput
        ...
        <div className="flex justify-end">
          <GuestButton
            ...
            disabled={busy || !current || !passwordRuleOk(next) || next !== confirm}
          >
            {busy ? '변경 중…' : '비밀번호 변경'}
          </GuestButton>
        </div>
