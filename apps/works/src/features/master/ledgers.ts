...
export const LEDGERS: Record<LedgerKey, LedgerMatchSpec> = {
  startups: {
    ...
    retired: (row) => Boolean(row.deleted_at),
  },
  networks: {
    table: 'networks',
    columns: 'id, name, affiliation, email, phone, deleted_at, merged_into_id',
    matchColumns: { name: 'name', email: 'email', phone: 'phone' },
    // 통합 원장은 중복 병합(정본으로 흡수)을 운용하는 유일한 원장이다 — 흡수된 행은 더는
    // 스스로를 답하지 않으므로 비활성과 같은 무게로 본다.
    retired: (row) => Boolean(row.deleted_at || row.merged_into_id),
  },
  ma_sellers: {
    table: 'ma_sellers',
