...
import { ProgramFilters } from '@/features/ac/ProgramFilters'
import { ProgramTable } from '@/features/ac/ProgramTable'
import {
  EMPTY_PROGRAM_FILTERS,
  useDeactivateProgram,
  useProgramsPage,
  type ProgramFilters as Filters,
} from '@/features/ac/programsPoolHooks'
import type { Program } from '@/features/ac/hooks'

/** 목록 페이지당 행 수(서버 사이드 페이지네이션). */
const PAGE_SIZE = 30

interface AcWorkspaceTabProps {
  /** 'mine' = 내가 담당자/등록자인 사업만, 'all' = 전체 사업. */
  scope: 'mine' | 'all'
}

/**
 * AC 워크스페이스: 프로그램 원장 목록.
 * 검색어(프로그램명·등록자)·복수 필터(상태·시작일)·서버 페이지네이션·다중선택·비활성화를 소유하고,
 * 검색창과 필터를 한 컨트롤 행으로 함께 배치한다. (STARTUP StartupPoolTab과 동일 구조.)
 * scope='mine'이면 등록자(created_by)·담당자(program_managers)가 현재 사용자인 사업만 조회한다.
 */
export function AcWorkspaceTab({ scope }: AcWorkspaceTabProps) {
