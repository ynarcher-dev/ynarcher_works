...
export function useProgramsPage(
  ...
  page: number,
  pageSize: number,
  /** 지정 시 등록자(created_by) 또는 담당자(program_managers)가 이 사용자인 사업만 조회한다('내 사업'). */
  mineUserId?: string | null,
) {
  return useQuery({
    queryKey: ['ac', 'programs', 'page', keyword, filters, page, pageSize, mineUserId ?? null],
    ...
    queryFn: async (): Promise<ProgramPage> => {
      ...
      // '내 사업' 필터: 등록자(created_by=나) OR 담당자(program_managers.user_id=나).
      ...
      let q = supabase
        .from('programs')
        .select(PROGRAM_COLS, { count: 'exact' })
        .is('deleted_at', null)
