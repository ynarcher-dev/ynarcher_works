...
import { AI_CARDS, AI_CARD_KEYS, type AiCardKey } from '@/features/startup/startupAiCards'
import {
  cellCount,
  cellOn,
  cardCountFor,
...
describe('칸·줄·열 토글', () => {
  ...
  it('카드 묶음은 묶음 안의 카드만 함께 켜고 끈다', () => {
    const group: AiCardKey[] = ['basics', 'summary', 'business', 'tech']
  ...
  it('전체 토글은 빈 격자를 다 채우고 채워진 격자를 비운다', () => {
    ...
    expect(cellCount(full, CARDS)).toBe(CARDS.length * KEYS.length)
...
describe('격자에서 요청을 뽑는다', () => {
  it('한 칸이라도 켜진 카드만 작성 대상이다', () => {
    expect(gridCards({ business: ['a'], tech: [], team: undefined }, CARDS)).toEqual(['business'])
  })

  it('작성 대상은 체크한 차례가 아니라 화면 순서로 선다', () => {
    // 순서가 흔들리면 같은 조합인데 서버가 다른 프롬프트를 만든다.
    const grid: AiGrid = { investment: ['a'], basics: ['a'], tech: ['a'] }
    expect(gridCards(grid, CARDS)).toEqual(['basics', 'tech', 'investment'])
  })

  it('자료는 카드가 몇이든 한 번만 센다(한 번 올려 함께 쓴다)', () => {
    const grid: AiGrid = { business: ['a', 'b'], tech: ['b'], team: ['a'] }
    expect(gridSourceKeys(grid, CARDS).sort()).toEqual(['a', 'b'])
  })

  it('빈 격자는 카드도 자료도 없다', () => {
    expect(gridCards({}, CARDS)).toEqual([])
    expect(gridSourceKeys({}, CARDS)).toEqual([])
    expect(cellCount({}, CARDS)).toBe(0)
  })
})
...
describe('pruneGrid — 사라진 자료를 걷는다', () => {
  it('없는 자료를 가리키는 칸을 지운다', () => {
    ...
    const pruned = pruneGrid({ business: ['a', '지워진자료'], tech: ['지워진자료'] }, ['a'], CARDS)
    expect(pruned.business).toEqual(['a'])
    expect(pruned.tech).toBeUndefined()
    expect(gridCards(pruned, CARDS)).toEqual(['business'])
  })

  it('전부 사라지면 빈 격자다', () => {
    expect(pruneGrid({ business: ['x'] }, ['a'], CARDS)).toEqual({})
  })
})
