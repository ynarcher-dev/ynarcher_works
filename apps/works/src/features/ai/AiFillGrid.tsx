...
/**
 * 왼쪽 네 칸의 폭 — 파일명 23% · 형식 4.5% · 용량 6% · 분석 상태 8.5%.
 *
 * 파일명이 가장 넓지만 그마저 4분의 1 아래로 묶는다. 이름은 잘려도 커서로 답할 수 있지만,
 * 카드 열은 잘리면 무엇을 켜는 칸인지 알 수 없다. 나머지 셋은 담기는 값의 **가장 긴 경우**에
 * 맞춘다(형식 `XLSX`, 용량 `999 KB`, 상태 `재분석 필요` + 아이콘) — 값이 짧은 칸을 넓게
 * 두면 그 여백만큼 카드 열이 좁아지고, 좁아지는 쪽은 언제나 잘리면 안 되는 쪽이다.
 *
 * 19%에서 올렸다(2026-09-08 사용자 지적) — 참조 자료에 붙는 위치 태그가 같은 칸에서 폭을
 * 가져가 이름이 두 글자만 남는 줄이 생겼다. 카드 열이 그만큼 좁아지지만 대화 폭도 함께
 * 넓혔으므로(`modal-3xl` 112rem) 절대 폭으로는 카드 칸도 종전보다 넓다.
 */
const NAME_COL = 'w-[23%]'
const EXT_COL = 'w-[4.5%]'
const SIZE_COL = 'w-[6%]'
 ...
 */
const LEFT_COLS_PCT = 23 + 4.5 + 6 + 8.5
