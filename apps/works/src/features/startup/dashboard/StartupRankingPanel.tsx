...
export function StartupRankingPanel({
  ...
  return (
    <div className="space-y-2">
      {shown.length === 0 ? (
        <p className="py-8 text-center text-caption text-gray-600">표시할 기업이 없습니다.</p>
      ) : (
          ...
          <tbody>
            {shown.map((r, i) => (
              ...
              >
                ...
                <td className="py-1.5 text-gray-700">{r.stage}</td>
                <td className="py-1.5 text-right tabular-nums text-gray-800">{won(r.valuation)}</td>
                <td className="py-1.5 text-right tabular-nums text-gray-700">{won(r.funding)}</td>
      ...
      {hidden > 0 && <p className="pt-0.5 text-right text-caption text-gray-700">외 {hidden}개</p>}
    </div>
  )
}
