...
/** 금액 셀(천원 단위, ÷1000 반올림). 음수(적자·자본잠식)는 빨간색 ▼로 강조. 단위는 카드 헤더에 표기. */
function Won({ v }: { v?: number | null }) {
  ...
  if (n < 0) return <span className="text-danger">▼{Math.abs(n).toLocaleString()}</span>
  return <span>{n.toLocaleString()}</span>
}

/** 최신 N개년만 노출(연도 내림차순으로 이미 정렬됨). 카드 폭이 좁아 3개년으로 제한. */
function recent(metrics: GrowthMetric[], n = 3): GrowthMetric[] {
...
const FINANCE_SERIES: ChartSeries[] = [
  ...
  { key: 'liabilities', name: '부채', color: C.gray4 },
  { key: 'equity', name: '자본', color: C.gray3 },
]
const REVENUE_SERIES: ChartSeries[] = [
  { key: 'revenue', name: '매출액', color: C.brand },
  { key: 'operatingProfit', name: '영업이익', color: C.gray5 },
  { key: 'netIncome', name: '당기순이익', color: C.gray3 },
]
...
const INVESTMENT_SERIES: ChartSeries[] = [
  ...
  { key: 'fundingAmount', name: '투자유치액', color: C.gray4 },
]
...
/**
 * 성장 지표 섹션(읽기): 재무/매출/고용/투자 현황 표 + 막대차트.
 * 비즈니스 타임라인은 별도 카드(StartupBusinessTimeline)로 분리해 주주 구성 아래에 둔다.
 * 편집은 통합 수정 폼에서 관리하므로 카드별 수정 버튼·수정 날짜는 두지 않는다.
 */
export function StartupGrowthSection({ metrics }: Props) {
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {/* 재무 현황 */}
        <Card title="재무 현황 (최신 3개년)" unit="단위: 천원">
          {rows.length === 0 ? (
            <Empty text="등록된 재무 정보가 없습니다." />
          ) : (
            <>
            <StartupMetricChart data={chartRows} series={FINANCE_SERIES} />
        ...
        {/* 매출 현황 */}
        <Card title="매출 현황 (최신 3개년)" unit="단위: 천원">
          {rows.length === 0 ? (
            <Empty text="등록된 매출 정보가 없습니다." />
          ) : (
            <>
            <StartupMetricChart data={chartRows} series={REVENUE_SERIES} />
        ...
        {/* 고용 현황 */}
        <Card title="고용 현황 (최신 3개년)">
          {rows.length === 0 ? (
            <Empty text="등록된 고용 정보가 없습니다." />
          ) : (
            <>
            <StartupMetricChart data={chartRows} series={EMPLOYEE_SERIES} unit="count" />
            <Table head={<><th className={thL}>연도</th><th className={th}>고용 인원</th></>}>
              {rows.map((m) => (
                <tr key={m.year}>
                  <td className={tdL}>{m.year}</td>
                  <td className={td}>
                    {m.employeeCount == null ? <span className="text-gray-400">-</span> : `${Number(m.employeeCount).toLocaleString()}명`}
                  </td>
                </tr>
              ))}
            </Table>
        ...
        {/* 투자 현황 */}
        <Card title="투자 현황 (최신 3개년)" unit="단위: 천원">
          {rows.length === 0 ? (
            <Empty text="등록된 투자 정보가 없습니다." />
          ) : (
            <>
            <StartupMetricChart data={chartRows} series={INVESTMENT_SERIES} />
            <Table head={<><th className={thL}>연도</th><th className={th}>기업 가치(Pre)</th><th className={th}>투자유치액</th><th className={thL}>라운드</th><th className={thL}>투자자</th></>}>
              {rows.map((m) => (
                <tr key={m.year}>
                  <td className={tdL}>{m.year}</td>
                  <td className={td}><Won v={m.valuation} /></td>
                  <td className={td}><Won v={m.fundingAmount} /></td>
                  <td className={tdL}>{m.fundingRound || '-'}</td>
