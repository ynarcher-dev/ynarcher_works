...
/**
 * 권역·투자단계가 비어 있는 행이 모이는 키. 서버가 같은 문자열을 내려주며,
 * 두 축의 '미지정' 타일이 이 키로 건수를 읽고 필터로도 이 축을 건다.
 */
export const FACET_UNSET = 'UNSET'

/** 축별 집계. 권역 키는 권역 태그 id, 투자단계 키는 단계 태그명이다. */
export interface StartupFacetCounts {
   ...
   * 권역·단계가 둘 다 걸려 있으면 합이 다르다(권역 축의 합은 '권역을 풀었을 때',
   * 단계 축의 합은 '단계를 풀었을 때'의 수다). 한 값으로 합치면 한쪽 카드가 자기 것이
   * 아닌 수를 말한다.
   */
  regionTotal: number
  stageTotal: number
...
/**
 * 기업 목록 요약 카드(권역별·투자단계별 현황)의 집계.
 *
 * 두 카드가 **같은 훅을 부르고 캐시 키도 같아** 요청은 한 번만 나간다. 종전 구분 카드는
 * 타일마다 목록 조회를 한 번씩(5회) 쐈고, 같은 방식으로 두 줄(약 19칸)을 세우면 요약만으로
 * 스무 번 가까이 호출하게 된다.
 *
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<StartupFacetCounts> => {
      ...
      for (const row of (data ?? []) as { axis: string; key: string; cnt: number | string }[]) {
        const target = row.axis === 'stage' ? stage : region
        target.set(row.key, Number(row.cnt))
      }
      const sum = (m: Map<string, number>) => [...m.values()].reduce((a, b) => a + b, 0)
      return { region, stage, regionTotal: sum(region), stageTotal: sum(stage) }
    },
  })
}
