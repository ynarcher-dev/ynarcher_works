 ...
 * 우측(1/3): 자료 관리 → 관련 회의록 → 변동 이력 → 코멘트(상세 공통 순서).
 * 전자결재는 2026-09-14에 우측에서 좌측 탭 줄(예산/지출 오른쪽)로 옮겼다 — 근거는 탭 줄 주석과
 * `WorkspaceApprovalTab` 주석에 있다.
      ...
      <div className="min-w-0 space-y-4 lg:col-span-2">
        ...
        <Tabs
          items={[
            ...
            // 나가므로, "얼마가 배정됐나" 다음 질문인 "무슨 결재가 오갔나"는 한 칸 옆이 맞다.
        ...
        {tab === 'workflow' ? (
          ...
          // 결재 연동의 대상 키도 회의록·예산과 같은 워크스페이스별 entityKey다
          // (program / ma_program) — 하나를 공유하면 PROJECT 결재가 M&A 딜에 붙어 보인다.
          <WorkspaceApprovalTab targetType={config.entityKey} targetId={program.id} />
        )}
      </div>
      {/* 우측(1/3): 자료 관리 → 관련 회의록 → 변동 이력 → 코멘트.
          급한 순서가 곧 위에서 아래 순서다 — 일하러 들어온 사람이 먼저 찾는 것은 자료고,
          코멘트는 다 보고 남기는 말이라 맨 아래에 둔다. 상세 화면 전부가 이 순서를 공유한다.
          **전자결재는 2026-09-14에 여기서 빠져 좌측 탭 줄로 갔다** — 이 컬럼은 일을 둘러싼
          것들(자료·회의록·이력·말)이 서는 자리이고, 결재는 이 사업 안에서 하는 일 자체다.
          한 벌을 두 곳에 세우지 않으므로 우측에 복제해 두지 않는다. */}
