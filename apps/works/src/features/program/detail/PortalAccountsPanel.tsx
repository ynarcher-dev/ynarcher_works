...
/**
 * 사업 상세의 `Y&A 포털 계정생성` 탭 — 자격을 하위 탭으로 가르고 그 아래 명부가 선다.
 *
 * **왜 자격을 상위 탭에서 내렸는가**(2026-09-08 사용자 지정). 2026-09-05에는 반대로 올렸고
 * 그때의 근거는 "자격은 표를 거르는 조건이 아니라 게스트에게 **다른 화면을 여는 축**"이라
 * 필터처럼 읽히면 안 된다는 것이었다. 하위 탭은 필터가 아니라 여전히 탭이므로 그 근거는
 * 상하지 않는다 — 달라진 것은 **탭 줄이 답해야 하는 물음**이다.
 *
 * 상단 탭 줄은 '이 사업에 무엇이 있는가'를 종류로 답한다(워크플로우 = 모듈, 사업개요·공지·
 * Q&A = 게스트에게 나가는 화면). 그 줄에 자격 둘이 나란히 서면 여섯 칸 중 둘만 같은 종류의
 * 다른 값이라, 훑는 눈이 종류와 값을 번갈아 읽어야 한다.
 *
 * **M&A에서는 그것이 결함이 된다.** 그 상세에는 이미 `SELLER`·`BUYER` 탭이 있고 그것은
 * *연결된 기업의 내용*이다. 명부 자격까지 같은 층에 올리면 같은 낱말이 한 줄에 두 번 서서
 * 서로 다른 것을 가리킨다 — 눌러 보기 전에는 어느 쪽이 무엇인지 화면이 답하지 못한다.
 * 층을 하나 두면 `SELLER`(그 기업)와 `포털 계정생성 > SELLER`(그 기업에서 들어오는 사람)가
 * 이름이 아니라 **자리로** 갈린다.
 *
 * 2026-09-05에 실제로 잘못이었던 것은 층이 아니라 **이름**이었다 — 그때의 상위 탭은
 * `참가자`였고 그 아래 `참가자`·`전문가`가 다시 서서, 같은 말이 부모와 자식에 겹쳐 무엇이
 * 무엇을 담는지 말하지 못했다. 지금의 이름은 그 아래 것들과 겹치지 않는다.
 *
