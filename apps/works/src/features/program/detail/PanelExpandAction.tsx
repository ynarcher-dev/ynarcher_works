import { TextAction } from '@ynarcher/ui'

/**
 * 우측 패널(참가자 풀·통합 타임라인) 헤더의 '전체 보기 →' 액션.
 * 좌측 뒤로가기 링크와 동일한 브랜드 텍스트 톤을 써 상세 페이지 전반의 링크 스타일과 통일한다.
 * `onExpand` 미지정 시 아무것도 렌더하지 않는다(개요 외 컨텍스트에서 재사용 안전).
 *
 * 톤을 직접 적지 않고 공용 `TextAction`에 맡긴다 — 예전에는 같은 클래스 문자열을 여기에 한 벌
 * 더 적어 두어, 링크 색을 한 번 바꾸면 이 자리만 옛 색으로 남을 수 있었다.
 */
export function ExpandAction({
  onExpand,
  label = '전체 보기',
}: {
  onExpand?: () => void
  label?: string
}) {
  if (!onExpand) return null
  return (
    <TextAction arrow="right" onClick={onExpand}>
      {label}
    </TextAction>
  )
}
