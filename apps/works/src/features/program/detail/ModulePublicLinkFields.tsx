 ...
 * 모듈 세팅 창의 '링크 공유' 칸.
 *
 * 공유 범위 칸 아래에 서지만 **다른 층**이다 — 위는 이 모듈이 어디까지 나가는가(성격,
 * ADMIN이 템플릿에 지정)를, 여기는 그 문이 지금 열려 있는가(운영)를 말한다. 둘을 하나로
 * 합치지 않는 이유는 "밖으로 나가는 모듈인데 지금은 닫아 둔 상태"가 공고 게시 전·마감 후의
 * 정상 상태이기 때문이다.
 *
 * 칸이 서는지는 **템플릿의 성격**이 답한다(module_templates.visibility = 'PUBLIC_LINK').
 * 그 성격이 아닌 템플릿에서는 칸 자체를 두지 않는다 — 꺼진 채 비활성으로 두면 언젠가 켜질
 * 수 있는 것처럼 읽히는데, 그것을 정하는 자리는 여기가 아니라 ADMIN 모듈 관리다.
 *
 * 칸이 선 뒤 **켜고 끄는 것은 담당자**다. ADMIN은 종류의 성격을 정했을 뿐이고, 이 건을 지금
