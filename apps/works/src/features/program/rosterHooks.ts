 ...
 * 두 벌로 답해야 한다. (2) 가리킬 원장 행이 없는 줄은 자료 참조·AI 작성 등 원장 행을
 * 전제로 하는 업무를 수행할 수 없다. 담을 수는 있는데 아무것도 못 하는 줄이 생긴다.
 * (3) 회의록 외부 참석자
 * 간이 등록이 같은 갈림길에서 이미 원장에 넣는 쪽을 골랐다.
 *
 * 받는 칸은 표에 서는 넷뿐이고 **넷 다 필수**다(2026-09-10). 이는 업무 원장의 연락처
 * 완결성 규칙이며 GUEST 계정 생성 계약과 무관하다. 이름 하나만 아는 대상은 명단이 아니라
 * 원장에서 먼저 만든다.
 *
