...
export const APPROVAL_BOX_GROUPS: ApprovalBoxGroup[] = [
  {
    ...
    boxes: [
      ...
      // 확인: 참조자로 지정된 내가 아직 확인 체크를 누르지 않은 문서. 상세를 열었다는 사실과
      // 확인 의사를 갈라, 내 이름 옆의 회색 체크를 직접 눌러야 이 칸에서 빠진다.
 ...
 * 상태 축이고 '확인'만 참조 확인 축이다.
 *
 * 참조 확인은 결재 여부와 별개로 내가 직접 읽고 확인 의사를 남겨야 하는 할 일이다. 그래서
 * 대시보드에서도 진행 상태들과 함께 신호를 세운다.
 *
 * 참조자이면서 결재자일 수 있으므로 '확인'은 진행 상태와 겹칠 수 있다. 대시보드 합계를 낼 때는
 * 문서 id 기준으로 중복을 제거해야 한다.
 *
...
/**
 * 결재선 구분. **셋 다 자기 구분 안에서 순차로 흐른다** — 앞 순번이 처리해야 다음 차례가 온다
 * (2026-08-26 확정. 종전에는 합의·재무합의만 병렬이었다).
 *
 * 구분끼리는 서로를 기다리지 않는다. 세 구분은 각자 하나의 줄이고, 문서는 그 줄 셋이 나란히
 * 흐르다 마지막 하나가 끝나는 시점에 완료된다 — 합의 줄을 결재 줄의 앞뒤 어디에 못 박으면
 * 결재선 표가 말하는 세 자리가 사실은 한 줄이라는 뜻이 되어, 구분을 나눈 이유가 사라진다.
 *
 * 참조는 결재하지 않으므로 이 축이 아니라 별도 원장(approval_recipients)이 담는다. 순번을
 * 갖지 않는 유일한 자리이기도 하다.
 */
