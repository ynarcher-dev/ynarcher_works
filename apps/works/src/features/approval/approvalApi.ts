...
const FORM_SELECT =
  'id, name, abbrev, retention, security_grade, is_active, sort_order, current_version_id, ' +
...
export interface ApprovalDetail {
  ...
  form: { name: string; retention: string; security_grade: string } | null
...
const DETAIL_SELECT =
  ...
  'form:form_id(name, retention, security_grade), ' +
  'version:form_version_id(fields), ' +
  'approval_lines(id, approver_id, step_order, decision, comment, decided_at), ' +
...
export function useCreateApproval() {
  ...
  return useMutation({
    mutationFn: async (v: CreateApprovalInput): Promise<string> => {
      ...
      if (v.approverIds.length > 0) {
        const { error: le } = await supabase.from('approval_lines').insert(
          v.approverIds.map((approver_id, i) => ({
            document_id: doc.id,
            approver_id,
            step_order: i + 1,
          })),
