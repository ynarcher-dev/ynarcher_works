...
interface ApprovalDecideBarProps {
  ...
  /** 지금 처리해야 할 내 결재선 행. */
  lineId: string
  /** 내가 마지막 결재자인가(승인 시 문서가 최종 승인으로 끝난다). */
  isFinal: boolean
}
 ...
 */
export function ApprovalDecideBar({ documentId, lineId, isFinal }: ApprovalDecideBarProps) {
  ...
  return (
    <Card
      title="결재 처리"
      subtitle={isFinal ? '내가 마지막 결재자입니다 — 승인하면 문서가 완료됩니다.' : undefined}
    >
