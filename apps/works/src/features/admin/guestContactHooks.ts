 ...
 * 비밀번호 초기화 확인 문구.
 *
 * 화면에 흩어 적지 않고 한 곳에서 소유한다 — 초기화가 무엇을 하는지(최초 비밀번호로 되돌린다)를
 * 말하는 문장이고, 그 사실이 바뀌면 고칠 자리가 하나여야 한다.
 */
export const GUEST_PASSWORD_RESET_CONFIRM =
  `비밀번호를 초기화하시겠습니까? 초기화하면 최초 비밀번호가 다시 ${GUEST_INITIAL_PASSWORD}가 되고, ` +
  '다음 로그인에서 본인이 개인 비밀번호를 정해야 화면에 들어갑니다. 안내는 발송하지 않습니다.'
...
/** 프로필 상한. 서버 검증과 같은 값이라야 판정이 갈리지 않는다. */
const NAME_MAX = 100
 ...
 * 계정 정보 수정 입력 검증.
 *
 * 계정 프로필인 이름·이메일·소속과 감사 사유를 검증한다. 연락처는 계약에 없다.
 */
export function validateGuestContact(input: {
  ...
  else if (name.length > NAME_MAX) errors.name = `이름은 ${NAME_MAX}자를 넘을 수 없습니다.`

  if (!email) errors.email = '이메일(로그인 ID)을 입력하세요.'
  else if (email.length > 254 || !EMAIL_RE.test(email))
    errors.email = '이메일 형식이 올바르지 않습니다.'

  if (!affiliation) errors.affiliation = '소속을 입력하세요.'
  else if (affiliation.length > AFFILIATION_MAX)
    errors.affiliation = `소속은 ${AFFILIATION_MAX}자를 넘을 수 없습니다.`
...
export function hasGuestContactError(errors: GuestContactErrors): boolean {
  return Boolean(errors.name || errors.email || errors.affiliation || errors.reason)
}
 ...
 *
 * 판정을 모아 두는 이유는 둘이 **함께** 걸리기 때문이다 — 권한과 진행 중인 요청. 화면에서
 * 두 조건을 버튼마다 따로 적으면 한 곳을 고칠 때 다른 곳이 남는다.
 *
 * **초기화에 선행 조건은 없다**(2026-09-14). 종전에는 연락처가 없으면 막았는데, 그것은
 * 연락처가 초기 비밀번호이던 시절의 규칙이다 — 최초 비밀번호가 고정값이 된 뒤로는 연락처가
 * 비어도 초기화된 계정으로 들어올 수 있으므로 막을 근거가 사라졌다.
 *
...
export function guestAdminActionState(input: {
  ...
  return {
    ...
    canResetPassword: visible && !input.pending,
  }
}
...
export interface GuestContactUpdateResult {
  ...
  affiliation_changed: boolean
  session_version: number
  /**
   * 서버가 **실제로 저장한** 세 값. 아무것도 쓰지 않았으면(`changed: false`) null이다.
   *
   * 서버는 바뀐 값을 되돌려주지 않으므로 여기서 채운다. 추측이 아니라 계약이다 —
   * 서버는 우리가 보낸 값을 정규화해 세 칸을 함께 저장한다.
   */
  applied: { name: string; email: string; affiliation: string } | null
}
...
>(snapshot: T | null, result: GuestContactUpdateResult): T | null {
  ...
  return {
    ...
    affiliation: result.applied.affiliation,
  }
}
...
/**
 * 두 창구가 함께 무효화할 조회 키 — 목록과 명부의 사람 고르기가 같은 프로필을 세운다.
 *
...
function invalidateGuestQueries(qc: ReturnType<typeof useQueryClient>) {
  return Promise.all([
    ...
    qc.invalidateQueries({ queryKey: ['guest-account-candidates'] }),
  ])
}

/**
 * 이름·로그인 아이디(이메일)·소속 수정.
 *
 * 원장(스타트업·네트워크·M&A)은 건드리지 않는다 — 계정의 로그인 자격과 표시 정보는 `users`가
 * 갖고, 원장 연락처는 그 원장의 창구가 따로 고친다.
 */
export function useUpdateGuestContact() {
  ...
  return useMutation({
    mutationFn: async (v: {
      ...
      const affiliation = v.affiliation.trim()
 ...
 *
 * 그 뒤의 로그인은 고정 최초 비밀번호(`GUEST_INITIAL_PASSWORD`)로 들어와 **개인 비밀번호를
 * 정한 뒤에야** 세션을 받는다. 안내는 보내지 않는다 — 자격증명은 오프라인으로 전한다
 * (3_9_1 §6).
 */
