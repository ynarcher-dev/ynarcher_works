...
export interface MaPartyRow {
  ...
  /**
   * 담당자 연락처. 계정이 아니라 원장이 갖는다(2026-09-08) — 발급 폼에서 담당자가 직접 적게
   * 하면 같은 사실이 두 곳에 살고, 번호가 바뀐 날 어느 쪽이 정본인지 답할 근거가 없다.
   *
   * **자격증명이 아니다**(2026-09-14 — 초기 비밀번호가 고정값으로 옮겨졌다, 3_9_1 §6).
   * 계정을 세울 때 기본값으로 옮겨 담길 뿐이고, GUEST 계정에서 연락처는 선택 칸이다.
   */
  phone: string | null
  /**
