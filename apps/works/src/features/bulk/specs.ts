...
export const STARTUP_BULK_SPEC: BulkImportSpec = {
  ...
  guide:
    '기업명만 필수이고 나머지는 비워 두어도 됩니다. 구분은 화면에서 쓰는 말(발굴기업·보육기업·미지정 기업)을 그대로 적으면 되고, 비워 두면 미지정 기업으로 들어갑니다. 투자기업은 업로드로 만들 수 없습니다 — 투자기업 전환은 FUND 투자 집행이 처리합니다. 분야·단계·소재지는 ADMIN 태그 관리에 등록된 이름만 받으며, 분야는 세미콜론(;)으로 최대 3개까지 적을 수 있습니다. 주주·성장지표처럼 기업 한 곳에 여러 줄이 붙는 정보는 등록 후 상세 화면에서 입력합니다.',
  // 구분을 적지 않은 줄은 '미지정 기업'으로 들어온다(2026-09-06). 종전 기본값은 발굴이었는데,
  // 파일에 적히지 않은 것을 발굴로 단정하면 발굴기업 수가 실제보다 커진다 — 모르는 것은
  // 모르는 칸에 담고, 무엇인지 정해지면 담당자가 옮긴다.
  fixedValues: { management_status: 'other' },
