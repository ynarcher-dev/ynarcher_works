 ...
 *
 * 주 색(1번)은 브랜드 액센트 인디고(`brand` 토큰 `#2E5CB8`)를 사용한다 — STARTUP 차트와 동일한 톤.
...
/**
 * 범주형 다계열 팔레트. 1번=브랜드 인디고(주 색), 이후 chart.2~8. 순서대로 순환 적용한다.
 * 브랜드가 청색 계열이 되면서 기존 chart.1 Blue(`#2563EB`)와 계열이 겹치므로, 6번 슬롯은
 * 적색(`#DC2626`)으로 교체해 인접 색상 간 판별을 유지한다.
 */
export const CHART_CATEGORICAL = [
  '#2E5CB8', // brand — 단독 데이터·가장 중요한 1계열(브랜드 인디고)
...
/** 라벨 연결선 — gray.300. */
export const CHART_LABEL_LINE = '#D9DEE5'
 ...
 * 순서대로(index) 순환 적용한다(0번=브랜드 인디고). 세로·가로 막대 공용 — 같은 정렬 배열이면
