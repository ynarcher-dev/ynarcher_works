 ...
 *
 * 밀도 규칙은 "카드 안에 든 표는 카드가 말하는 주제의 부속이라 한 단 내린다"였고, 그 한 단이
 * 13px이던 시절 이 카드에서는 그것이 카드를 두 크기로 갈랐다 — 비었을 때의 문장은 본문
 * 단(14px)이고 행이 들어오면 13px이라, 데이터가 채워지는 순간 같은 자리의 글자가 작아졌다.
 * 행이 표의 한 줄이 아니라 **문장에 가깝다**는 것이 이유다(소속 조합 한 덩이 + 괄호로 이은
 * 경위). 좌측 본문 열의 카드라 폭도 표처럼 아껴 쓸 자리가 아니다.
 *
 * 2026-09-06에 13px 단이 폐지되어 두 단의 글자 크기는 같아졌다(남은 차이는 자간·셀 여백뿐).
 * 그래도 이 선택은 그대로 둔다 — 고른 근거가 "어느 크기인가"가 아니라 "이 카드의 행은 표가
 * 아니라 문장이다"라는 자리 판단이었고, 단이 다시 갈릴 날 이 카드는 다시 본문 쪽에 서야 한다.
 *
